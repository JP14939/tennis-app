"""
Convert net_geometry_labels_v2.json into YOLO-pose training format, mirroring
prepare_yolo_pose_dataset.py's exact structure (the approach that worked for
the racket keypoint model, unlike the direct-regression approach the original
net_keypoints model used).

4 keypoints: net_top_left, net_top_right, left_post_base, right_post_base.
Post-base points are frequently absent (occluded/cropped) -- those get
visibility=0 in the label line (loss ignores them) rather than being dropped,
so frames with only the top corners visible still contribute training data.

Bounding box is derived from whichever keypoints are present (min/max + padding).
"""
import json
import os
import random
...
DATASET_DIR = r'C:\Users\jackp\tennis_app\data\10_net_detection\yolo_pose_dataset_v2'
...
def main():
    ...
    n_val = max(4, int(len(usable) * VAL_FRAC))
    ...
    for split, items in splits.items():
        ...
        for item in items:
            ...
            if 'net_top_left' not in present or 'net_top_right' not in present:
                continue  # need at least both top corners to be useful
            ...
            for p in POINTS:
                if p in clamped:
                    ...
                    kp_str += ' 0.000000 0.000000 0'  # absent -- visibility 0, loss ignores it
