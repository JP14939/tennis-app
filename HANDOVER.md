...
## Scheduled-routine PR round-up (2026-09-10)

Checked via the GitHub MCP tools (`list_pull_requests`/`search_pull_requests`,
no `gh` CLI in this environment) for PRs created in the last 24 hours against
`jp14939/tennis-app`. Three of today's five scheduled routines opened a PR
overnight — **PRs #39–#41, all still open**, none merged (these routines
never merge their own work). **No `🚨 CRITICAL:` PR today** — the security
review explicitly states no CRITICAL finding. No future-ideas/brainstorm PR
today and no training-data-drift-watch PR either — both expected: both run
on a weekly-Mondays cadence (per `TODO_MANUAL.md`'s 2026-09-07 entry, the
drift-watch routine is actually weekly-Mondays too, not the every-3-days
table `HANDOVER.md`'s 2026-08-26 session recorded — the two have drifted out
of sync with each other and this is the more reliable source), both last
fired 2026-09-07, next due 2026-09-14. All PRs from every prior round
(#17–#38) are already merged/closed (see the 2026-09-07 `pr-merge` note
above), so today's 3 are the **only** open PRs in the repo right now.

- **PR #39 — "Logic review 2026-09-10: fix serve contact-detection drift in
  1v1 compare"** (branch `logic-review/2026-09-10`, opened 03:06 UTC).
  `compare_videos.py`'s 1v1 premium comparison (`/api/compare-videos`) calls
  the same `build_user_trajectory()` helper the pro-database match path
  uses, but never passed `shot_type` through — so its auto-detect path
  always fell back to the plain wrist-velocity peak, serves included,
  missing the overhead-apex serve-anchor fix `compare_swing.py` already
  has. A serve-vs-serve premium comparison with no manual contact mark on
  one or both sides was silently using the less-accurate heuristic. Fix:
  pass `shot_type=shot_type` through on both `build_user_trajectory()`
  calls in `compare_videos()` — a 1-file, +8/-2 diff. Reviewed every backend
  route plus `compare_swing.py`/`phase_breakdown.py`/all of
  `scripts/09_coaching_ai/` with no other changes made. 620/620 backend
  tests passing; Python change verified by `py_compile` only (no
  `scripts/venv` in the sandbox) — flags wanting a real `pytest` run before
  merge given it touches the live comparison path.
- **PR #40 — "Bug sweep 2026-09-10: date-rollover validation, duplicate reel
  clips, tied match scores"** (branch `bug-sweep/2026-09-10`, opened 03:27
  UTC). Three fixes, each with a regression test: (1) `isIsoDateTime()`
  (`invariants.js`) accepted calendar-invalid dates like `"2026-04-31"` —
  JS's lenient `Date` parser silently rolls them over (→ May 1st) instead of
  rejecting, so the stored literal string permanently disagreed with
  anything that re-parses it later; now re-derives year/month/day and
  confirms they round-trip through `Date.UTC` unchanged. (2)
  `POST /highlights/jobs/:id/reel` didn't dedupe `rallyIds`, so a repeated
  id (e.g. a UI double-tap) stitched the same clip into the output reel
  twice and persisted the duplicate; now deduped before use. (3)
  `POST /friends/:userId/matches` validated `setsWon`/`setsLost`
  independently but never against each other, so a tied score (e.g. 2–2)
  was accepted and silently credited neither player's win/loss record; now
  400s on a tie. Also fixed for future-proofing (no live trigger yet):
  `compare_swing.py`'s pro-clip contact-time lookup used a truthy check
  instead of `is not None`, which would silently drop a legitimate `0.0`
  contact time once `use_roi_tracker` is ever enabled. 625/625 backend
  tests passing (+5 new). Same standing caveat as #39 on the one Python
  change — no `scripts/venv` in this sandbox to run real `pytest`.
- **PR #41 — "Security review 2026-09-10: rate-limit GET /courts's Overpass
  self-heal, patch multer DoS advisories"** (branch `security-review/2026-09-10`,
  opened 03:40 UTC). Re-audit focused on the surface added by the
  2026-09-06/07 Find Games + rally-shots merge, since the last security pass
  (2026-09-07 03:40 UTC) predated it landing on `master`. Two fixes: (1)
  `GET /courts`'s Overpass self-heal (added by that merge) had no rate
  limit, unlike every other route that triggers outbound/heavy work — an
  authenticated caller varying `lat`/`lng` by >~1.1km per request (e.g.
  walking through open-ocean coordinates) could defeat the existing
  coordinate-bucket cache and force a fresh live Overpass call (up to 25s)
  on every request, unbounded, risking the shared production IP getting
  rate-limited/banned by Overpass for every real user. Fixed with the same
  user-id-keyed `rateLimit` pattern the ML-subprocess routes already use
  (30 req/5 min). (2) `npm audit` (not run recently) surfaced 2 high-severity
  advisories: `multer` ≤2.2.0 (DoS via crafted multipart field names/array
  indices, an fd leak on aborted uploads, a file-size-limit bypass via a
  `fileFilter` race — live-reachable on every upload route) and `js-yaml`
  3.0.0–3.15.1 (transitive dev-only, via `jest`, not production-reachable).
  Both patched via `npm audit fix` within existing semver ranges
  (lockfile-only). Re-verified auth/JWT, SQL parameterization, every
  `spawn` call site, secret handling, and IDOR scoping on the new
  watch/confirm/availability/per-shot-analyze endpoints — all clean, no new
  issue. 620/620 backend tests passing; `npm audit` clean after fix.

Not merging any of these myself per standing instructions — same as every
prior round-up, they're left for Jack's review. Nothing in today's three PRs
revealed a new manual-only to-do beyond "review and merge #39–#41" — the
one open structural item (training-data-drift-watch's no-data-access gap)
was already logged to `TODO_MANUAL.md` on 2026-09-07 and needed no update
today.
