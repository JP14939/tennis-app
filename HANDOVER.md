...
**Current state (2026-09-11, after commit `2f750e6`):** the reviewed ML
reliability, eval-hygiene, review-tool, and documentation work is committed
and pushed. The five recurring risks remain accurately described: DTW score
calibration is still bench-only and provisional; audio contact detection is
strong (~89% within 3 frames) while the no-audio visual fallback is weaker
(25% on real amateur footage versus 38% on broadcast); rebuilt ML/database
artifacts still need manual transfer because CD does not deploy `data/`;
commit-discipline and eval-hygiene guardrails are now documented and landed.
The remaining data transfer and device/release checks are operational TODOs,
not an uncommitted code pile.

**User:** Jack Price (jack.p14370@gmail.com)
**Project root:** `C:\Users\jackp\tennis_app\`

**Quick status (2026-09-02):** built, hosted, and auto-deploying (Hetzner + Docker, CD on push to `master`), feature-complete well past MVP, **pre-launch — no external user has ever used the live product**. Core loop + payments + social/gamification all live. All eleven backlogged scheduled-routine PRs (#17–#27) merged and deployed 2026-09-02 in one batch — `users.token_version` JWT revocation, per-user ML-route rate limits, ~15 bug/crash-guard fixes, path redaction, and integrity-check expansion (17→42 orphan canaries) are now live. Open items that matter: the `detect_rallies.py` serve-gate bug (blocks rally/highlight grouping), the shot-classifier retrain, and a real beta. (Update 2026-09-05: serve-gate fixed 2026-09-03, classifier retrain landed 2026-09-04/05 — see STATUS.md for what's actually still open.) For the current curated snapshot read `STATUS.md`; this doc is the full dated history.
...
| # | Feature | Status |
|---|---|---|
| 1 | Unlimited Uploads (2/day free vs. paid) | **Built (update — this row was stale).** `backend/src/utils/usageLimit.js` (`reserveDailyUsageSlot` / `releaseUsageSlot`, `FREE_DAILY_LIMIT = 2`) is enforced in `analyse.js` against a real `isPremium`; a failed analysis releases its slot so a genuine error doesn't burn one. Since 2026-09-02 a per-user rate limiter (`analyseLimiter`, 30/10min) also caps total attempts regardless of outcome. |
...
- **Update (stale — fixed):** `API_BASE` is no longer a hardcoded LAN IP. `frontend/config/api.js` now exports `API_BASE = process.env.EXPO_PUBLIC_API_BASE || 'https://rallymax.167-233-107-31.sslip.io'` — defaults to the hosted backend so any device works without config; local dev overrides via `EXPO_PUBLIC_API_BASE` in `frontend/.env`.
...
**Real, standing gap this surfaced**: there is no CI/CD. Every push to GitHub needs a manual `ssh` in, `git pull`, `docker compose up --build app` on the server (plus a manual data copy if `data/` changed) to actually go live — confirmed this is not automatic. See `TODO_MANUAL.md` for the exact commands. — **Update (2026-08-25): closed. `.github/workflows/deploy.yml` (item #44) now auto-deploys code on every push to `master`; only `data/` transfers and `.env` edits stay manual.**
...
*(section reflects file structure as of ~2026-08-10; the app was restructured in several later sessions — a few entries below were corrected in place during a 2026-09-05 pass, but treat this as historical rather than live-current, and see the "Navigation structure" section above and dated session entries below for what actually changed)*
...
│   ├── App.js                             # Tab nav (now Home/History/Friends/FindGames/Profile, 5 tabs — Drills folded into History and Premium off the tab bar, per items 19 and 38 below) + stack screens, wrapped in AuthProvider
...
│   ├── config/
│   │   └── api.js                         # API_BASE (defaults to hosted backend, overridable via EXPO_PUBLIC_API_BASE — no longer a hardcoded LAN IP)
│   ├── context/
...
│       ├── (DrillsScreen.js deleted — Drills content now lives in components/DrillsSection.js, folded into History, item 19)
│       ├── PremiumScreen.js               # now gates on tier; no longer a standalone tab, folded into Home (item 38)
...
## Scheduled-routine PR round-up (2026-09-04)

Checked via the GitHub MCP tools (`search_pull_requests`/`list_pull_requests`,
no `gh` CLI in this environment) for PRs created in the last 24 hours, plus
`list_branches` as a cross-check. Three routines opened a PR overnight —
**PRs #28–#30, all still open**, none merged (these routines never merge
their own work). **No `🚨 CRITICAL:` PR today** — the security review
explicitly states no CRITICAL finding. No brainstorm/future-ideas PR today
(weekly-Mondays cadence, last one was 2026-08-31, next due 2026-09-07). No
scheduled-routine PRs at all on 2026-09-02 or 2026-09-03 — also expected:
the code-review routines (logic/bug/security) run every 3 days and last
fired 2026-09-01, so 2026-09-04 is their next scheduled day, not a gap.
All ten PRs from the prior three rounds (#17–#27) are already merged (see
the 2026-09-02 batch-merge note above), so today's 3 are the **only** open
PRs in the repo right now — a much cleaner starting point than the
ten-deep backlog called out in the 2026-09-01 round-up.

- **PR #28 — "Logic review 2026-09-04: 8 correctness/invariant fixes"**
  (branch `logic-review/2026-09-04`, opened 03:14 UTC). Eight fixes:
  (1) `highlights.js`'s `runJob()`/`runReelJob()` had no guard against the
  double-event (`'error'` + `'close'`) spawn-failure mode `runPythonJson.js`
  already defends against elsewhere, and no `SIGKILL` escalation after a
  timeout — both now match the established pattern. (2) `history.js`'s
  `PATCH /history/:id` silently dropped a non-boolean truthy
  `flagged_not_shot`/`confirmed_real_shot` (e.g. the string `"true"`)
  instead of 400ing. (3) `friends.js`'s share-notification fired again on
  a duplicate/retried share instead of only on a real insert. (4)
  `rateLimit.js` was actually a fixed/tumbling window despite its own
  comments describing a sliding one — a caller could burst up to 2x the
  intended cap across a window boundary, directly relevant since this is
  the only guard on `/auth/login`/`/auth/signup`/`/auth/forgot-password`;
  replaced with a weighted sliding-window-counter approximation. (5)
  `courts.js`'s Overpass empty-result self-heal cache was keyed on
  coordinates only, not radius, so a narrow-radius miss could suppress a
  later wider-radius search that might've found real courts; now always
  self-heals at `MAX_RADIUS_KM`. (6) `dev.js`'s drill step-reconciliation
  silently let a duplicate submitted step id overwrite itself twice
  instead of rejecting; now 400s. (7) `drills.js`'s `GET /drills` validated
  `kind` but not `shot_type` against the shared vocabulary, unlike every
  sibling route. (8) `compare_swing.py`'s live `racket_body_distance` was
  sampled at a different density (`sample_every=3`) than the pro-database's
  offline-precomputed value (`sample_every=4`) it's diffed against — now
  matched. Investigated and deliberately reverted one candidate fix to
  `find_peak_wrist_frame()` after confirming it broke an existing test
  whose intent contradicted the proposed change (see the PR body's
  "Investigated, not changed" section). 517/517 backend tests passing.
  Python change verified by hand (no `cv2`/`mediapipe` venv in this
  sandbox) — same recommendation as always to run real `pytest` before
  merging given it touches the live comparison path.
- **PR #29 — "Bug sweep 2026-09-04: NaN-fps crash, orphaned upload dir,
  missing FK checks, notifications_enabled coercion"** (branch
  `bug-sweep/2026-09-04`, opened 03:31 UTC). Four fixes, cross-checked
  against PR #28 to avoid duplicating its rate-limiter/SIGKILL fixes: (1)
  `compare_swing.py`'s fps-validity guard (`not fps or fps <= 0`) let a
  `NaN` fps (a real OpenCV/FFmpeg quirk on some malformed videos) slip
  through and crash later with a raw Python `ValueError` instead of the
  intended friendly "may be corrupted" message — added `math.isnan(fps)`
  to the guard. (2) `analyse.js` didn't clean up the partially-written
  `USER_CLIPS_DIR/<uploadId>/` directory when `persistAndCrop()` failed
  partway through, unlike `compareVideos.js`'s equivalent path — now
  mirrors it. (3) `integrityChecks.js`'s `ORPHAN_CHECKS` was still missing
  4 real FK columns (`coach_links.coach_id`/`student_id`,
  `coach_invite_codes.student_id`, `push_tokens.user_id`) despite its own
  header comment claiming full coverage — added. (4) `auth.js`'s
  `PATCH /auth/me` had no type check on `notifications_enabled`, so a
  truthy non-boolean like the string `"false"` would silently coerce to
  *enabled* — the opposite of the caller's evident intent; added the same
  `typeof === 'boolean'` check the other two patchable fields already have.
  522/522 backend tests passing. Same standing caveat as #28: the
  `compare_swing.py` fix needs a real `scripts/venv` pytest pass before
  merging (no `cv2`/`mediapipe` in this sandbox).
- **PR #30 — "Security review 2026-09-04: backend auth/JWT/upload/SQL/
  subprocess audit"** (branch `security-review/2026-09-04`, opened 03:46
  UTC). Full re-audit of auth, JWT/session handling, every upload/multer
  call site, every `spawn`/`runPythonJson` call site, SQL construction, and
  secret handling — confirms everything from 5+ prior rounds (JWT algorithm
  pinning, `token_version` revocation, per-user ML rate limits, upload
  extension whitelisting, admin path-traversal fixes) is still in place.
  **No CRITICAL finding.** Two real fixes: (1) `qs` (transitive, via
  express/multer/body-parser) was pinned to `6.15.3`, carrying two moderate
  GHSA advisories (array-limit bypass, `isBuffer` DoS) reachable pre-auth
  on any endpoint's query string — added a `qs: ^6.16.0` override,
  `npm audit` now clean. (2) `GET /courts` accepted out-of-range
  lat/lng (unlike `POST /courts`, which already validates via
  `isLatitude`/`isLongitude`), letting any authenticated caller trigger a
  garbage-bounding-box Overpass lookup; now validated the same way as the
  write path. 520/520 backend tests passing (4 new).

**Not merging any of these myself per standing instructions** — same as
every prior round-up, they're left for Jack's review. Nothing in today's
three PRs revealed a new manual-only to-do beyond "review and merge
#28–#30" — see `TODO_MANUAL.md`.
## Scheduled-routine PR round-up (2026-09-02) — the big batch merge

All eleven backlogged PRs (#17–#27, three review rounds + the future-ideas
doc + two docs round-ups) were reviewed, merged, tested, and deployed in
one session. This was run **with a real dev environment** (working
`scripts/venv` with `cv2` 5.0.0 + `mediapipe`, a real copy of the
production `app.db`) — so the Python and migration checks the routines'
own sandboxes could never do actually got done here.

**Method.** The PRs all branched from the same `master` commit and the
later rounds re-fixed bugs the earlier rounds already fixed (the
`compare_swing.py` wrist-visibility gate was in three PRs; the
`infer_angle.py` decode guard and the `compareVideos.js` orphan-file fix
in two each; `integrityChecks.js` touched by four). GitHub showed all
eleven "mergeable" but that is pairwise-vs-`master` only. So rather than
merge them straight, a `pr-merge/2026-09-02` integration branch was built
in a separate `git worktree` off `master`: eleven `--no-ff` merges in
dependency order, and at every overlap a single **canonical owner** was
picked and the other PRs' duplicate hunks dropped in-conflict (resolved
to `master`'s side for that hunk). Every PR still merged and still shows
**MERGED** on GitHub — each just contributed only its non-duplicate part.

**Canonical-owner decisions:**
- `compare_swing.py` `find_peak_wrist_frame()` visibility gate → **#25**
  (it also carries the only regression test,
  `scripts/08_comparison_engine/test_compare_swing_pytest.py`). Dropped
  from #20 and #24.
- `infer_angle.py` `extract_frame()` decode guard → **#25** (covers all
  three call sites; #18 only two). Dropped from #18.
- `compareVideos.js` orphaned-file cleanup → **#25** (`Promise.allSettled`
  + synchronous `fs.rmSync` + rethrow, cleaner than #18's async
  `fs.rm`). Dropped from #18.
- `integrityChecks.js` `ORPHAN_CHECKS` expansion (17→42) + `club_watches`
  entries + the corrected "FK enforcement is OFF" header comment → **#25**
  (strict superset). Dropped from #24. #18's *different* additions
  (`drill_routine_steps.shot_type`, `coach_notes` structural) kept; #20's
  duplicate `drill_routine_steps.shot_type` dropped.
- The false "better-sqlite3 enables `PRAGMA foreign_keys` by default"
  comments in `auth.js` / `history.js` / `dev.js` → #24/#25's corrected
  wording wins. **#17's own added comment (and its test comment) claimed
  the same false thing and was rewritten during the merge** — the
  `DELETE /auth/me` child-before-parent delete order it fixes is still
  correct and becomes load-bearing the moment FK enforcement is turned on
  (tracked backlog item), it just doesn't throw *today*.

**Non-trivial conflict resolutions:**
- `auth.js` `DELETE /auth/me` — kept **all four** overlapping changes in
  the one transaction: #17's delete reorder, #22's `token_version` bump,
  #24's `club_watches` cleanup, #25's stale-reset-token invalidation.
- `auth.js` `/auth/reset-password` — kept **both** #22's `token_version`
  bump on the `password_hash` UPDATE and #25's broader
  `WHERE user_id = ? AND used_at IS NULL` reset-token invalidation.
- `analyse.js` — git integrated #21's two-`try` split (Python-call
  failure releases the usage slot; post-processing failure does not) with
  #25's pro-clip `fs.statSync` existence check (now *inside* the second
  `try`) and #26's `analyseLimiter` on the route line. Added a
  `timeout:` entry to the error-`messages` map for the new
  `kind: 'timeout'` from #25's `runPythonJson.js` SIGKILL escalation.
- `history.js` `stripHeavyOverlays` — #21's non-array-`matches` guard +
  #25's `if (!result) return result` null guard + `safeJsonParse` all
  coexist. `history.test.js` — #21's tests renumbered `history10/11` to
  avoid an email collision with #25's `history8/9`.
- `docs/future-ideas.md` — #23's `### 2026-08-31` section ordered below
  the manually-added `### 2026-09-01` competitor-analysis section.
- Every PR branch's own HANDOVER/TODO doc edits were dropped in-conflict;
  this section is the single consolidated round-up instead.

**One extra commit on the integration branch** (`a694e37`): #25's own
SIGKILL-escalation test spawned `bash -c "trap … sleep 30"`, which on
Windows orphans the `sleep` child (it keeps a stdio pipe open, so
`'close'` never fires and the test hangs to its 10 s limit). Swapped the
stubborn child to a Node process with a no-op `SIGTERM` handler.
`runPythonJson.js` itself unchanged. Also bumped `/analyse`'s rate-limit
cap 20 → 30 during the #26 merge (a heavy legit drill session could brush
20) and updated its test.

**Testing (all green on the integration branch):**
- `cd backend && npm test` → **516 / 516**, 30 suites.
- `npm run verify:db` against a copy of the real production `app.db` →
  **83 / 83 invariants hold** — the expanded `ORPHAN_CHECKS` surfaced no
  pre-existing orphaned rows.
- #22 migration against the same copy (16 users) → `token_version INTEGER
  NOT NULL DEFAULT 0` added via `db.js`'s existing boot-time
  `PRAGMA table_info → ALTER TABLE` pattern, all rows backfilled to 0,
  idempotent on a second boot, no row loss.
- `cd scripts && pytest` with the real venv → **63 / 63**, including
  #25's new `test_compare_swing_pytest.py`.
- A real end-to-end `compare_swing.compare()` on a real clip → exit 0,
  full JSON shape intact, sane values (similarity 49.3 / overall 67.3 /
  contact 1.0 s), the new visibility gate did not break normal contact
  detection.

**Deploy.** `pr-merge/2026-09-02` fast-forwarded onto `master` as a
single push → one `deploy.yml` run (**33609545825**, 6 m 40 s, health
check passed). Post-deploy: prod `/health` OK; `/api/auth/login` with bad
creds returns a clean 401 (not a 500), confirming the `users` table +
auth path are healthy after the schema migration. The eleven PR branches
were then deleted and the integration worktree removed.

**Follow-ups still open (unchanged by this batch):**
- The `detect_rallies.py` `apply_serve_gate()` bug — flagged again in
  #24/#25, still **not fixed**, still needs Jack's call. Blocks the
  rally-grouping / highlight-clip feature and skews the shot-classifier
  training data serve-heavy.
- `train_shot_classifier_model.py` still not re-run on the corrected
  102-row dataset.
- `billing.js` is now **upgrade-only** (#21) — there is no longer any
  code path that can downgrade a user client-side. A lapsed subscriber
  whose `EXPIRATION` webhook is permanently lost keeps `premium`
  silently. Revenue leak, not user harm; a periodic reconciliation job
  would be the real fix.
- `PRAGMA foreign_keys` is still OFF (tracked separately, needs a full
  DELETE-site audit before flipping).
- Still no CI test gate before `deploy.yml` — a red suite still ships.
  This batch was hand-tested green before the push; the next routine
  round won't be unless someone adds the gate (`docs/future-ideas.md`
  2026-08-26 logs it).

---

## Session 2026-09-02 (later) — contact-frame accuracy + shot classifier (Sprint 0/1 of the ML-reliability plan)

**Nothing in this session is committed yet.** Large uncommitted surface — the
new scripts below plus the pre-existing untracked Pro Clip Review tooling.
Work is checkpointed here mid-flight at Jack's request. Working plan:
`C:\Users\jackp\.claude\plans\okay-where-do-things-woolly-pond.md`.

### 1. The contact frame the whole pipeline hangs on was never measured — so we measured it

`scripts/07_ball_racket_tracking/eval_pro_clip_contact.py` (new) runs the live
pose/ball contact pipeline against the 197 pro clips that carry a real
hand-marked contact time (`contact_time_corrected` verdicts). Result:

| | median error | within ±3 frames |
| --- | --- | --- |
| live pipeline (`find_peak_wrist_frame` → `find_contact_frame`) | **9 frames** (~150 ms @ 60 fps) | **25 %** |

Why it's that bad:
- `find_peak_wrist_frame` finds peak *hand speed*, which happens in the
  follow-through — **~13 frames, biased ~+7 f late**, not at impact. It's a
  swing detector, not a contact detector, and it's the only signal for ~half
  the clips.
- `find_contact_frame`'s ball-occlusion-gap / ball-racket-proximity refinement
  is ~1.6 f early-biased and, given a *perfect* anchor, still lands 3–4 f off —
  it makes a good estimate worse.
- `data/07_ball_racket_tracking/contact_frame_model.pkl` is broken —
  extrapolates to 10 000+ frame offsets on this footage (trained on 75 rows
  with 4/12 features all-NaN). Currently gated behind a trust check so it's not
  actively hurting live analyses, but it's dead weight.
- **`clip_contact_time_sec = 1.001` on every pro-DB entry is not a random
  placeholder** — every clip was cut at `swing-detector-peak − 1.0 s`, so
  contact sits at 1.0 s *by construction*, and that peak is ~20 f off the truth.
  The DTW trajectory anchoring for every un-reviewed pro entry is genuinely
  wrong, not just unlabelled.

Memory: `project_contact_frame_predictor_eval`.

### 2. Audio-onset contact detection — the fix, 9 f → 0.75 f

Pro clips have **no audio stream**, but the source compilations
(`data/01_source_videos/**`) do (AAC 44.1 kHz). The ball-on-strings "pock" is
the sharpest, least-occludable event in the recording.

- `scripts/07_ball_racket_tracking/audio_onset.py` (new) — pure-DSP shared
  module (numpy + scipy + bundled `imageio_ffmpeg` only, **no new dependency**):
  ffmpeg audio extract → STFT → spectral flux → onset peak-pick →
  per-onset feature vectors (attack slope, spectral flatness/centroid/rolloff,
  HF/LF ratio, decay, rms, …).
- `scripts/07_ball_racket_tracking/eval_audio_contact.py` (new) — same
  eval harness shape as the pose eval. Finding: the correct onset is present in
  the detected set within **±3 f in 86 % of clips, ±5 f in 99 %**. A consistent
  **−2.5 f** detection latency (STFT frame-centre + humans marking slightly
  early) is corrected in code.
- `scripts/07_ball_racket_tracking/train_onset_classifier.py` (new) — a
  per-onset classifier that scores *how impact-like an onset sounds* **and**
  *how well it agrees with the video* (distance to the wrist-speed peak / the
  pose contact estimate — Jack's "near the wrist peak + a loud sharp sound =
  contact" intuition, which the fitted coefficients confirm). GroupKFold CV
  over 4 466 onsets / 197 clips:

| | median &#124;err&#124; | ≤1 f | ≤2 f | ≤3 f | p90 |
| --- | --- | --- | --- | --- | --- |
| **onset classifier pick** | **0.75 f** | 66 % | 85 % | **89 %** | 3.15 f |
| pose pipeline (baseline) | 9 f | 11 % | 19 % | 25 % | 58 f |

  Onset-level AUC 0.981. Model: `data/07_ball_racket_tracking/onset_classifier.pkl`
  + `_meta.json`.

### 3. Wired into the live app (Phase B.1) — additive, guarded

- `scripts/07_ball_racket_tracking/audio_contact.py` (new) —
  `detect_contact(video_path, anchor_time_sec, video_hints, search_window_sec,
  model_path, conf_proba, conf_margin) -> dict | None`. Loads the classifier
  (module-cached, version-aware via meta), applies the bias, `confident` gate
  (proba ≥ 0.60 & margin ≥ 0.20). Returns `None` cleanly on no audio stream /
  no model / no onsets / any exception.
- `scripts/08_comparison_engine/compare_swing.py` — `compare()` now, **on the
  auto-detect path only** (`contact_time_sec is None` — i.e. the user did *not*
  mark a contact frame), tries `detect_contact()` restricted to ±0.5 s of the
  wrist-velocity peak and uses it **only when the classifier is confident**,
  otherwise falls through to `find_peak_wrist_frame` exactly as before. **A
  user's manual contact mark is still never second-guessed.** Fully
  try/except-wrapped — an audioless upload behaves identically to today.
- `scripts/07_ball_racket_tracking/test_audio_contact_pytest.py` (new) —
  no-audio → None, no-model → None, junk path → None, model-present shape.
  Full `pytest` green.
- **Not yet verified end-to-end** on a real phone upload with an audio track
  (none available locally) — see TODO_MANUAL.

### 4. Phase C — "audio teaches a visual-only model" (for uploads with no audio)

Jack's constraint: the teacher generating training labels must not share the
student's model or features, so it can't just teach the student a better copy
of itself.
- `data/07_ball_racket_tracking/onset_classifier_audioonly.pkl` (new) —
  pure-audio teacher, **14 audio-shape features only** (no pose, no ball, no
  clip-cut time prior). On its confident picks (~52 % of clips) it's
  93 % within ±3 f — clean labels.
- `scripts/07_ball_racket_tracking/build_contact_student_dataset.py` (new) —
  for each pro swing, take a contact label from **(a)** Jack's hand mark
  (`contact_time_corrected` only) or **(b)** the pure-audio teacher when
  confident, then extract the *visual* student's own evidence (pose →
  wrist-peak anchor → `track_racket_and_ball` → `find_contact_frame` +
  `contact_frame_meta` + a new wrist velocity/accel/jerk profile) and log a
  training row. Teacher = audio, student = vision → disjoint.
- **Done: the human-label pass.** `contact_frame_training_log.jsonl` now holds
  **196 real hand-marked contact rows + 75 real user-submitted marks** + 21
  older review rows.
- `scripts/07_ball_racket_tracking/train_contact_frame_model.py` (repurposed):
  `TRAIN_SOURCES = {user_submitted, human, audio_teacher}`, the dead
  `pro_clip_review` constant-`1.001` rows dropped, 5 wrist-kinematics features
  added, GroupKFold by source video, `--reg gb|huber`, a human-rows-only
  accuracy slice in the report.
- **Not yet done:** the audio-teacher labelling pass (would add ~500 rows from
  the pro source footage), the student retrain, and C.4 (wire the student into
  `compare_swing` for the audioless path + audio/visual fusion).

### 5. Bug: `label_confirmed` no longer means "contact verified"

In the *original* review workflow `label_confirmed` meant "contact time + shot
type both checked". In the **quality-only** workflow (below) Jack used it for
"video quality is fine" — those ~168 entries still carry the 1.0 s / 1.8 s
placeholder contact time. **Only `contact_time_corrected` (~196) are real
hand-marked contact frames.** Any script deriving a contact label from the
review log must filter to `contact_time_corrected`. Caught + fixed in
`build_contact_student_dataset.py` (80 poisoned rows purged). Shot *type* is
trustworthy on all label-review verdicts. Memory:
`project_pro_clip_review_progress`.

### 6. Pro Clip Review finished (Sprint 0)

Jack completed the quality-only pass over the whole live pool. **359 clips kept**
(forehand 215, backhand 123, **serve 21**), 199 excluded, 36 shot-type relabels.
Serve is very thin — 77 % of pro serves were excluded (bad framing / the
ball-visibility filter). The full-pool rebuild
(`rebuild_pro_database_from_verdicts.py` — still untracked) has **not been run**;
`pro_database.json` is still in its half-hand-corrected state.

Data-quality note from Jack during the review: **clips around swing_id ~2015
(forehand job 2) are filmed from the front** (camera at the net facing the
player), so their pose landmarks are mirrored vs. the behind-baseline clips —
handle at rebuild time.

### 7. Shot-type classifier retrain (Sprint 1) — investigated, not shipped

`scripts/14_shot_classifier/train_shot_classifier_model.py` still hadn't been
re-run since the 102-row fix. This session:
- **Body-normalised the pose features** — every magnitude feature (wrist
  separation, velocities, wrist-vs-shoulder heights) is now divided by torso
  length, so a phone selfie (player fills the frame) and a broadcast clip
  (player small) produce comparable numbers. `FEATURE_VERSION = 'v2-bodynorm'`;
  `classify_shot._get_ml_model()` refuses a model whose meta version doesn't
  match (falls back to the rule scorers) so a stale model can't silently get
  mis-scaled inputs. `test_extract_training_features_pytest.py` updated.
- Regenerated all three feature files.
  `extract_training_features_from_pro_verdicts.py` →
  `training_features_from_pro.json` = **402 rows, 151 backhand** (14× the old
  11-backhand amateur set).
- **Findings (held-out-amateur F1 — the honest metric; whole-pool CV is
  inflated by serve-heavy log rows):**
  - body-norm alone lifts backhand F1 0.30 → 0.40 (but n = 10 amateur backhand
    test rows — noise-level; don't over-read it).
  - The log-derived feature set (76 serve / 1 backhand) is **net-harmful** —
    drops held-out amateur 0.63 → 0.60. Retrain with the new `--no-log` flag.
  - **Pro data still hurts the amateur/live model** at every weight (backhand
    F1 → ~0.20) — negative transfer, same conclusion as 2026-08-27, even
    body-normalised. Pro technique / camera angles / one-vs-two-hand mix don't
    transfer to phone footage.
  - Pro data *does* make a good **pipeline-specific model** — amateur + pro
    whole-pool CV (pro rows in the test set = the broadcast-ish domain
    `detect_rallies` runs on) gives backhand F1 **0.63 on n = 161**.
  - **The real bottleneck is 10 amateur backhand training examples.** Jack
    sourcing more amateur backhand footage is the only thing that moves the
    live model.
  - **The serve over-prediction blocking rally detection is probably NOT a
    classifier problem.** `detect_rallies.py` → `get_verified_shot_type` → the
    Claude shot verifier (`shot_classifier_verifier.py`) is sent the
    **contact-frame image**, and the frame it uses is the swing-detector
    wrist-peak — ~13 f off. A groundstroke mid-swing (racket up and back)
    reads as a serve. Feeding `detect_rallies` an accurate contact frame
    (sections 2–4 above) is the likely fix; retraining the student can't help
    if the frames it's shown are wrong.
- **Nothing retrained or saved this session** — `shot_classifier_model.pkl` is
  still the old v1 model. Decisions for Jack in TODO_MANUAL.

### New files this session (all untracked)

```
scripts/07_ball_racket_tracking/
  audio_onset.py                     shared pure-DSP onset detection + features
  audio_contact.py                   detect_contact() -- live inference module
  eval_pro_clip_contact.py           pose/ball contact pipeline eval vs 197 human marks
  eval_audio_contact.py              audio onset eval vs the same 197
  train_onset_classifier.py          the onset classifier (+ --audio-only teacher)
  build_contact_student_dataset.py   audio/human labels -> contact_frame_training_log rows
  test_audio_contact_pytest.py
data/07_ball_racket_tracking/
  onset_classifier.pkl / _meta.json           the live fusion model
  onset_classifier_audioonly.pkl / _meta.json the Phase-C label teacher
  eval_pro_clip_contact.csv, eval_audio_contact.csv, ..._teacher_anchor.csv
```
Modified (also untracked/uncommitted): `compare_swing.py`,
...
> **Resolved 2026-09-04:** the run completed — 333 `practice_mvp` entries
> landed (verify_source `claude_verified`), 225 forehand / 70 serve /
> 38 backhand, ~$5 actual Claude spend. Jack chose to keep and manually
> review rather than revert; **201/333 reviewed as of 2026-09-04**, see
> "Session 2026-09-04" below for what's now wired up around this data.
...
### Part 3 — Phase C wiring finished (code only; runs + retrain still pending)

Picked up while the ingest ran (it needs no Claude credits and touches no
pro-DB file). Phase C = "the pure-audio teacher labels pro swings → train a
visual-only contact-frame student → use it on uploads that have **no audio
track**" (today those fall back to the wrist-velocity peak, ~9f median error).

- **New `scripts/07_ball_racket_tracking/contact_evidence.py`** — the visual
  contact pass (`wrist_kinematics` + `compute_contact_evidence`) factored out of
  `build_contact_student_dataset._student_evidence` into one shared module, so
  the offline dataset builder and the live `compare_swing` path run identical
  feature code. Uses the non-cropped `track_racket_and_ball` (matches the
  offset model's training distribution).
- **`compare_swing.compare()` — C.4 wired in.** After the audio-onset block, a
  new audioless fallback: pose wrist-peak anchor → `compute_contact_evidence` →
  `train_contact_frame_model.predict_contact_offset` → corrected contact time.
  **Gated on `contact_frame_ml_training_log.stats()['trusted']`** — until the
  model earns trust the block is a no-op and behaviour is exactly as today.
  `wrist_velocity_fallback` (no real racket/ball evidence) is also skipped. If
  the audio detector returned a *sub-threshold* pick and the visual student
  lands within 2f of it, the audio time is taken (audio + visual agreement).
  Fully try/except-wrapped, same as the audio block.
- **`build_contact_student_dataset.py --force-relog`** (new flag) — re-processes
  already-logged `human` swings so the pre-Phase-C 196 human rows get the 5
  `wrist_*` kinematics features the model now expects. `train_contact_frame_
  model.py` now dedups to the newest row per `swing_key`+`source`.
- **Tests:** `test_contact_student_pytest.py` (6 new). Full suite **183 green**.

**Still to run (Jack / an idle machine — no credits, but YOLO-heavy; start it
after the ingest + shot-verification runs finish so it isn't CPU-starved):**
1. `python 07_ball_racket_tracking/build_contact_student_dataset.py --audio-only`
   then `... --human-only --force-relog` (adds ~500 `audio_teacher` rows +
   backfills the human rows). Writes only `contact_frame_training_log.jsonl` +
   caches — no `pro_database.json` contact.
2. `python 07_ball_racket_tracking/train_contact_frame_model.py` — ship only if
   the `corrected (human)` slice beats `raw (human)` and the ~9f baseline.
3. Copy `data/07_ball_racket_tracking/contact_frame_model.pkl` +
   `contact_frame_model_meta.json` to the server (gitignored, like the pro DB).
   The live path stays a no-op until the model is present *and* trusted.

**Sprint 1 (shot-type classifier) — answer to "did we do it?":** yes, as far as
code goes (see §7 of the 2026-09-02 session). No remaining code task — it's
blocked on Jack sourcing amateur backhand footage (only 10 examples) and on
accurate contact frames, which Phase C + the `detect_rallies` contact wiring
already address.

### Part 4 — Sprint 2 (serve gate) fixed + a Claude-vs-ground-truth harness

**The two things breaking `detect_rallies` (0 rallies on `IMG_5755`):**
1. `apply_serve_gate()` used `(peak_time - last_time) > 6s` (the gap since the
   last *detected* swing) as a point boundary — but the swing detector misses
   most real rally shots, so that gap is a detection gap mid-rally, not a real
   pause. The gate slammed shut and cascade-dropped every following shot (all
   12 real forehands on IMG_5755).
2. Claude's shot-type picks in the pipeline are **78–79% `serve`** on rally
   footage (`ml_status_report.py`). Neither local classifier is trusted
   (rule 0.67 / ML 0.50 agreement *with Claude*), and the on-disk ML model is
   **inert** — trained pre-`v2-bodynorm`, rejected by `_get_ml_model()`'s
   version guard. So Claude runs on every swing, and we have **never measured
   Claude against real ground truth** (every label on disk is Claude's own or
   agreement-with-Claude).

**Sprint 2 fix (committed-pending, `detect_rallies.py` + tests):**
- New `POINT_BOUNDARY_GAP_SEC = 12.0`, separate from `RALLY_GAP_SEC = 6.0`
  (grouping only). The serve context now survives a missed shot or two.
- `apply_serve_gate(swings, rally_gap_sec, mode='advisory')` — **advisory** is
  the new default: keeps every non-serve shot, tags `sw['after_serve']` (bool),
  never cascades. `mode='strict'` = the old drop-until-a-serve behaviour (match
  footage only); `mode='off'` = ignore serve sequencing. `--serve-gate` CLI arg.
- `group_into_rallies` + rally clip bounds now use the audio-refined
  `contact_time_sec` (via new `_rally_time` helper) — the gate already did; they
  disagreed before.
- Rally dicts gain `opened_by_serve` (advisory confidence signal, not a filter);
  result JSON gains `serve_gate_mode`.
- `test_serve_gate_pytest.py` rewritten to the advisory contract; **189 pytest
  green**. Offline validation: `tune_rally_gap.py` (scores against Jack's
  `rally_clips.boundary_note` DB labels, no Claude).

**The Claude cross-check (harness built, not yet run):**
- `scripts/17_amateur_eval/evaluate_pro_review_labels.py` (new, mirrors
  `evaluate_amateur_dataset.py`). Ground truth = Jack's ~400 Pro Clip Review
  labels (`training_features_from_pro.json`, 151 backhand). Per clip records
  `rule_pick` / `ml_pick` (free) and, on a stratified `--claude-subset`
  (default 100, every backhand), `claude_primed_pick` (told the rule pick, =
  production behaviour) **and** `claude_blind_pick` (not told) — the split
  isolates "Claude is bad at this" from "Claude rubber-stamps the student".
  `shot_classifier_verifier.build_verification_prompt(blind=True)` added.
  Measurement only — **does not backfill** any training log.
- **To run:** `python 14_shot_classifier/extract_training_features_from_pro_verdicts.py`
  (regenerates the ground-truth file — currently absent), then retrain the local
  classifier so `ml_pick` works
  (`extract_training_features_from_log.py` → `train_shot_classifier_model.py --no-log`),
  then `python 17_amateur_eval/evaluate_pro_review_labels.py --claude-subset 100`
  (~$0.15 Haiku — **confirm spend first**).
- **Decision after the numbers:** separate `shot_classifier_pipeline_model.pkl`
  (amateur+pro, for the rally pipeline) vs just retrain the one model and run
  the pipeline with `RALLYMAX_SKIP_CLASSIFIER_VERIFIER=1`. If Claude blind is
  accurate, the bug is the priming — fix the prompt, keep Claude as teacher.

**Sprint 3** (re-run `detect_rallies` on IMG_5755, Claude-free) is unblocked once
the classifier path is chosen: `RALLYMAX_SKIP_CONTACT_VERIFIER=1
RALLYMAX_SKIP_CLASSIFIER_VERIFIER=1 python 11_highlight_clipping/detect_rallies.py
<IMG_5755> <out> --serve-gate advisory` — $0, expect `rallies_detected > 0`.

### Part 5 — shot classifier rebuilt (40% → 84% on the pipeline domain)

**Cross-check verdict (the paid run, ~$0.45 — botched, spawned 4 parallel copies,
now has a single-instance lock):** on 68 hand-labelled pro clips, **every**
method — rule, ML, Claude-primed, Claude-blind — scored ~35%, near the 33%
3-class floor. Claude blind never predicted "backhand" once in 44 tries. The old
classifier wasn't unlucky, it was broken; Claude wasn't a usable teacher for this.

**The reframing:** forehand vs backhand is a mirror-image distinction (which side
of the body the racket contacts on, defined by the dominant hand). Split the job:
- **serve vs groundstroke** — handedness-free (overhead reach at contact).
- **forehand vs backhand** — needs handedness (assume right-handed for now;
  per-player detection is Phase 2). Then a **view-invariant** geometric test:
  `dot(dominant_wrist − torso_mid, dominant_shoulder − torso_mid)` — a camera
  mirror flips both vectors so the sign is preserved (verified: front-view 65% =
  back-view 65%, no collapse).

**What shipped (all new, `scripts/14_shot_classifier/`):**
- `classify_shot_geom.py` — the geometric decision tree (serve gate → dot-product
  side test → two-handed-grip override), body-normalised, tunable thresholds.
- `classify_shot_trajectory.py` — distance-weighted k-NN vote over the pro-DB
  swing *trajectories* via the existing `dtw_distance`. **Excludes `ingest ==
  'practice_mvp'` entries** — the practice-footage ingest (which grew the DB
  415→618 mid-session) tags ~165/203 swings "forehand" and was polluting the
  pool (backhand recall 84%→74% until filtered).
- `classify_shot.classify_ensemble()` — geom serve gate + trajectory-kNN for
  FH/BH + geom fallback → `'uncertain'` (to Claude). `use_trajectory=False` for
  the phone path.
- `classify_shot_verified.get_verified_shot_type` — ensemble is now the first
  student, its own trust log (`shot_classifier_ensemble_training_log.py`). With
  `SKIP_CLASSIFIER_VERIFIER` it's used directly; otherwise **shadow-mode** logs
  it vs Claude so it earns the 0.90 trust gate on real pipeline data. Same
  shadow-logging added to `detect_rallies`.
- `scripts/17_amateur_eval/evaluate_shot_classifiers.py` — the comparison harness
  (leave-one-out on the 402 pro labels, cross-domain check on 116 amateur).

**Results (leave-one-out vs Jack's Pro Clip Review labels):**

| method | pipeline (pro, n=402) | phone (amateur, n=116) |
| --- | --- | --- |
| old rule | 40% (BH 24%) | 47% |
| geom alone | 63% (BH 51%, view-invariant) | **56%** (best on phone) |
| trajectory-kNN | 84% (FH 90 / BH 87 / **serve 15**) | 32% (BH 0% — pool is broadcast) |
| **ensemble** | **83.6%** (FH 88 / BH 84 / serve 44) | 53% |

- **Pipeline: the win.** 40%→84%, backhand 24%→84%. Ready to run (shadow mode, or
  directly under `SKIP_CLASSIFIER_VERIFIER`).
- **Serve is the soft spot (44%)** — a pose-quality ceiling on small broadcast
  players. The ensemble deliberately trades ~0.5pt overall for serve recall
  15%→44% (a serve mislabelled as a groundstroke pollutes rally grouping). Real
  fix = Phase 2 motion-direction features (racket path near-vertical, ball goes
  up), both already computed in the pipeline and discarded.
- **Phone: not ready.** Trajectory-kNN collapses cross-domain (every amateur
  backhand → forehand); geom-only ~56%. Phase 2 = motion features + the
  ~809 unlabelled amateur swings + retrain the (still inert v1) ML model.
- **NOT committed / pushed** — Jack's step. `train_shot_classifier_model.py`
  `--out-suffix` and the ML v2 retrain (Part 3 of the plan) were **not done** —
  the trajectory approach made a trained pipeline model unnecessary for now.

---

## Session 2026-09-04 — dev-workflow fixes, 5 classifier fixes, Phase C retrained (and rejected), tracker audit

Started as "why won't the web app load" and grew into a full session touching
local dev tooling, the practice-footage review pipeline, and both classifier
training flywheels. Everything below is uncommitted (working tree) unless
otherwise noted — 33 files touched, all pytest/jest suites green throughout.

### Local dev-workflow bugs (not app bugs, but real time sinks)

- **ngrok free-tier interstitial silently broke the web app.** `frontend/.env`
  pointed the web build at an ngrok tunnel (meant for phone testing); ngrok
  free serves browsers an HTML "click to continue" page with HTTP 200 instead
  of proxying, so every API call returned unparseable HTML — looked like
  "no swings in history" / a login loop, was actually zero backend
  involvement. Fixed: `.env` now points local web dev at `http://localhost:5000`
  directly (ngrok line commented for when phone testing resumes), plus a
  permanent `frontend/utils/installFetchShim.js` that adds
  `ngrok-skip-browser-warning` to every request so a tunnelled session can't
  silently break the same way again. `api/history.js` + `HistoryScreen.js`
  also hardened so a non-JSON 200 now throws visibly instead of blanking the
  screen.
- **All 429 practice-footage clips were unplayable in a browser.**
  `ingest_practice_footage.py`'s `extract_clip()` writes OpenCV's `mp4v`
  fourcc (MPEG-4 Part 2), not H.264 — every other clip-writing path in the
  pipeline re-encodes after `extract_clip` (`detect_rallies.py`,
  `crop_to_subject.py`, etc.); this one didn't. Fixed at the source (now
  calls `video_io.reencode_to_h264()` like the others) and backfilled all 429
  existing files in place via the existing `backfill_reencode_clips.py`
  (same frame count/fps/duration, contact times unaffected).
- **Pro Clip Review UX**: correcting shot type or contact time on a practice
  clip now stays on that clip (shows "Next →") instead of auto-advancing —
  practice clips almost always need both fixes, unlike the mostly-clean
  broadcast queue. Also fixed a stale progress counter (the "All done!"
  screen showed the count from when the batch loaded, not after review) by
  auto-loading the next batch when one runs out.

### Five shot-classifier / data-quality fixes, all tested

1. **Unreviewed practice entries excluded from live matching.** New
   `compare_swing.eligible_match_candidates()` — a `practice_mvp` entry only
   becomes a real match candidate once it has a Pro Clip Review verdict.
   Before this, all 333 unreviewed auto-labelled entries were live match
   targets the moment they landed in `pro_database.json`. Verified against
   the real DB: forehand pool 453→243, backhand 195→157, serve 100→36, all
   still well above the angle-filter's min-5 fallback.
2. **`serve_0180`-class serve-gate bug fixed.** Strong serve evidence
   (sustained overhead + above-head, or strong contact reach) now hard-wins
   over the forehand/backhand side test in `classify_shot_geom.classify_geom`
   *before* the softmax runs — previously a large lateral wrist offset at the
   marked (downswing) contact frame could mathematically outvote a correctly
   detected serve. Regression test added.
3. **User-correction training flywheel fixed** — `logShotTypeCorrection()`
   (`backend/src/routes/history.js`) was logging corrections with no
   `clip_path`/`contact_frame` at all, so `extract_training_features_from_
   log.py`'s first filter silently dropped **every** "Wrong shot type?"
   correction a real user ever made, since the feature shipped. Now resolves
   the row's saved clip + `contact_time_sec` and logs both; the extractor
   resolves the frame from the clip's own fps. The 35 pre-existing
   corrections are unrecoverable (never captured what they needed) —
   documented, not backfilled.
4. **Pro Clip Review → shot classifier flywheel fixed.**
   `extract_training_features_from_pro_verdicts.py` was excluding *all*
   `practice_mvp` entries outright — but by construction every entry
   reaching that filter already has a real human verdict (an unreviewed one
   has no verdict logged, so it's never a candidate in the first place). The
   blanket exclusion was simply wrong once review started. Deleting it was
   the whole fix. **119 reviewed rows already flowing into training** as of
   the last extractor run (43 forehand / 34 backhand / 42 serve) — the
   backhand count alone more than triples the old 10-example amateur
   ceiling that's been the phone-classifier bottleneck all along. Jack has
   since reviewed ~80 more; a re-run will pick those up.
5. **Pro Clip Review → Phase C (contact-frame model) flywheel built new** —
   didn't exist before. New `build_contact_student_dataset.py --practice`
   sweep reads Jack's practice-clip contact-time corrections from
   `clip_review_log.jsonl` and logs the tracker's own guess vs. the human
   mark as a training example (same `_student_evidence()` the broadcast
   sweep already uses). Deliberately a separate batch job, not hooked into
   `correct_contact_time.py`'s request path — that endpoint is awaited
   synchronously by the review screen and a ~10s MediaPipe pass per submit
   would make the review grind noticeably slower. **109 examples logged.**
   Hit and fixed a real bug along the way: the sweep re-read
   `pro_database.json` fresh on every single clip, which raced Jack's live
   review session's writes and crashed on a partial read — fixed to read the
   DB once per run.

### Phase C retrained twice — real negative result, not shipped

With the new data (audio-teacher sweep: 407 new rows, then +109 practice
`human` rows), `train_contact_frame_model.py` was re-run twice.
**Both times the "corrected" model was worse than the raw geometric
heuristic across every tolerance band** — e.g. `≤1f`: raw 37-38% vs.
corrected 10-26%. Diagnosed, not just observed: `audio_teacher` labels are
themselves noisy on broadcast audio (60% land as outliers vs. the geometric
guess, median 25f off even among "confident" picks) — but excluding them
entirely and training on human-only data was *also* worse, so it isn't just
noisy labels. The regressor isn't finding real signal in these features;
more of the same kind of data won't fix that, a different feature set / a
more conservative model / reframing the target would be needed. **Not
copied to the server** — also not live-consequential either way, since
`compare_swing.py`'s separate runtime trust gate requires 50+ real
production examples before it'll ever use this model, and that's at 0.

### Ball/racket tracker visual audit (new)

Built `render_contact_review_frames.py` — draws the tracker's own detected
ball/racket boxes on frames around its guessed contact frame *and* the
human mark, for a sample of reviewed practice clips, weighted toward
serves. Ran on 15 serve + 5 forehand + 5 backhand: median |error| in this
sample was similar across shot types (~17-19f), but **serves have a much
fatter error tail** — four separate misses over 85 frames (`practice_100126`
-107f, `practice_100109` +102f, `practice_100005` -119f, `practice_100103`
+86f) vs. groundstrokes' worst being -23f. Cross-checked against MediaPipe
wrist-landmark visibility on a small sample (12 clips) — serves looked fine
to good (0.81-0.89 median), no worse than groundstrokes — so this points at
the **ball/racket YOLO tracker**, not pose quality, as the likely root
cause for serves specifically (physically plausible: the ball travels much
higher/closer to the sky, the racket arc is faster and larger, more motion
blur at the critical frame). Output:
`data/07_ball_racket_tracking/contact_review/<clip_id>/*.jpg`. Not acted on
further this session — a real fine-tuning investment, flagged not started.

### Practice-footage review progress

**201 of 333 reviewed as of 2026-09-04** (Jack, ongoing in Pro Clip Review).
Broadcast queue essentially finished alongside it: 354/359 label-reviewed,
5 left with only a machine audio-fill to eyeball.

### Not done this session (flagged, not started)

- View-aware trajectory-kNN (the deferred "proper" phone-classifier fix —
  mirror the user swing to the pool's majority view, class-normalise the
  vote). ~1-1.5 days estimated, would push phone ~68%→~80%.
- The "auto-align the user's swing to whichever contact-time offset scores
  highest" idea Jack raised — discussed at length (confirmed cheap to try,
  since pose extraction is the expensive step and only needs to run once;
  flagged the real risk that naively maximising score would inflate every
  score rather than genuinely improve alignment) but not built — needs a
  bounded, self-consistency-based design, not a max-score search, if it's
  picked back up.
- Nothing pushed to the server or committed — same 24+-commit local backlog
  as before, now larger.

### Auth-convention route-manifest guard (new)

Built the guard `TODO_MANUAL.md`'s backend backlog had flagged as buildable
without Jack's input: `backend/src/domain/routeAuthExceptions.js` (explicit
allowlist of routes intentionally missing `requireAuth`/`optionalAuth` —
pre-login auth endpoints + the RevenueCat webhook, each with a `reason`) and
`backend/src/routeAuthConvention.test.js` (walks every router's real Express
middleware chain, not source text, and fails on any route missing both
unless listed). Verified it actually catches the bug class: temporarily
stripped auth from a route, test failed with a clear message naming it,
reverted. Full suite green throughout.

### Architecture review — highlights.js job-runner deepening (new)

Ran a full architecture review (`/improve-codebase-architecture`) over the
whole repo; report (self-contained HTML, Mermaid + hand-drawn diagrams) at
`%TEMP%\architecture-review-20260904-233151.html`. Four candidates surfaced:

- **A — done.** `highlights.js`'s `runJob()`/`runReelJob()` were shallow
  reimplementations of `runPythonJson.js`'s own interface, with both of its
  documented historical fixes (settled-guard against double DB writes/push
  notifications on a failed spawn; SIGKILL escalation against a child stuck
  in a blocking native call) only partially reapplied. Routed both through
  `runPythonJson` instead of hand-rolled `spawn`/timeout/stdout-stderr
  collection; extracted the one genuinely new bit (mapping a
  `PythonProcessError.kind` to a user-facing message, script's own reported
  error taking priority on `nonzero_exit`) into
  `backend/src/utils/resolveJobFailureMessage.js` with its own 6-case unit
  test. Full suite: 530/530. One visible behavior change: a timed-out job now
  gets its own distinct message instead of falling through to a generic one.
  **Note for whoever reconciles local `master` with `origin/master`:** origin
  already carries PR #28's shallower fix for the same bug (bare
  settled-flag + SIGKILL patch applied in place, deployed 2026-09-04) — this
  deepening supersedes it; take this version of `highlights.js`, not a merge
  of both.
- **B — not started, highest remaining leverage.**
  `scripts/08_comparison_engine/compare_swing.py`'s `compare()` (the single
  most-modified file in the whole repo) is a ~375-line function with 8 inline
  try/except special cases (audio contact, ML contact, camera angle/roll,
  handedness mirror, view direction, ball speed, two near-identical
  filter-with-fallback blocks, four overlay blocks) and no seams — every new
  detection strategy has structurally nowhere to go but another inline
  branch. Proposed deepening: a `ContactDetector` chain-of-strategies
  (audio → ML visual → heuristic), a `CameraGeometry` module (angle + roll +
  mirror + view-direction as one call), and a shared "filter pool with
  minimum-size fallback" helper. Bigger, riskier than A — needs its own
  grilling pass before touching it.
- **C — not started.** Three independent implementations of "find the
  contact/swing-peak frame" exist with no shared interface for the heuristic
  path: `compare_swing.py`'s own wrist-velocity peak finder,
  `detect_swings.py`'s separate wrist-velocity + percentile peak finder, and
  `racket_tracker.py`'s `find_contact_frame` (used two different hand-tuned
  ways by `contact_evidence.py` and `verify_shot_contact.py`). The ML/audio
  path already has this seam done right (`contact_evidence.py`, two real
  callers) — proof the pattern works when applied; the heuristic path never
  got the same treatment.
- **D — not started, smallest/safest next pickup.** Two contained
  duplications in the Pro Clip Review pipeline:
  `scripts/06_database_build/split_pro_clip.py` re-derives
  `rebuild_helpers.py`'s own documented canonical frame-math formula instead
  of calling `reextract_for_entry` like its two siblings
  (`correct_contact_time.py`, `rebuild_pro_database_from_verdicts.py`)
  already do; `source_footage_lookup.py`'s docstring admits it hand-copies
  `build_pro_database.py`'s `JOBS` list with a "keep in sync" comment and no
  test enforcing it.

No `CONTEXT.md` or `docs/adr/` exist in this repo yet — nothing to reconcile
findings against. Worth creating `CONTEXT.md` once domain vocabulary
(Pro Clip Review verdict, contact frame, etc.) gets pinned down during a
future grilling pass on B or C.

## Session 2026-09-05 — Find Games revamp: mesh clubs, watch system, postcodes, club naming

Full plan-mode writeup:
`C:\Users\jackp\.claude\plans\encapsulated-hugging-axolotl.md` (two rounds —
the watch-system plan, then extended in place for postcodes/naming). All
work below is uncommitted (working tree). Backend: 34/34 suites, 593/593
tests, `verify:db` 94/94 invariants (confirmed against the real local
`app.db`, including after a real 33k-row bulk write — see item 4).

### 1. Club clustering rewritten to a true node-mesh graph

`backend/scripts/clusterCourts.js`'s `clusterCourts()` used to grow each
cluster against a running centroid at 250m (a deliberate 2026-08-xx fix for
a worse bug: naive chaining had produced a 57-court mega-cluster). Replaced
with a real union-find connected-components graph at 100m: courts are
nodes, an edge connects two courts ≤100m apart, a club is one connected
component — confirmed directly with Jack that a straight line of courts
each ~90m apart is deliberately one club even though the two ends are
hundreds of meters apart (the literal "node mesh" reading, not a stricter
"every pair within 100m" clique). New edge case this introduces and now
handles: a stricter re-cluster can leave an old club with zero courts
(`club_courts` rows moved to a new `club_id`) — `deleteOrphanedClubs()`
cleans up the orphaned club and its now-meaningless `club_watches` rows.
`main()` was also made testable (`require.main === module` guard, ported
from `train_contact_frame_model.py`'s same fix this repo already applied) —
previously it ran unconditionally at module load, so nothing in this file
could be unit tested. 9 tests in the new `clusterCourts.test.js`.

### 2. A third watch type: arbitrary map areas

`area_watches` table (lat/lng + radius_km, no UNIQUE — unlike court/club
watches, a user can meaningfully want several overlapping areas). New
routes `POST/DELETE /courts/area-watch`; `POST /courts/:id/availability`'s
notification fan-out now also reaches any area watch whose radius reaches
the posting court, deduped into the same notified-user Set as court/club
watchers. Frontend: `FindGamesScreen.native.js` gained a "Watch an area"
mode — tap the header button, then tap the map to drop a pin with a live
radius circle (`react-native-maps`'s `Circle`) and a new continuous
drag-slider (`RadiusSlider`, generalizing `CourtSheet`'s existing 3-step
day-offset `PanResponder` slider to a continuous 0.1–20km range).

### 3. New "My Watches" screen + a real hydration bug fixed

There was previously no way to see or remove an existing watch except by
re-opening the exact court/club on the map — `getWatchedCourts` (backend:
`GET /courts/watched`) existed and returned the right shape but was never
called from any screen. New `frontend/screens/MyWatchesScreen.js` (three
sections: Courts/Clubs/Areas, each removable), reached from a new header
button on Find Games, registered as a `Stack.Screen` in `App.js`. Needed one
new backend route to make this work: `DELETE /courts/club-watch/:clubId`
(direct by club id) — the existing club-watch routes only ever resolved a
club from a *court* id, but the watched-clubs list has no associated court
id to work from.

Separately, a real bug fixed while wiring this up: `GET /courts` never told
the frontend which courts the user already watches at all — the map's
`watchedIds` Set always started empty on load, only becoming correct again
after re-opening that exact court's sheet (which derives club-watch state
correctly but had no per-court equivalent). Added `watched` to
`queryNearbyCourts`'s SELECT; the frontend now hydrates its Set from it on
every load.

Also extracted `haversineKm`/`boundingBox` (three near-identical copies
across `courts.js`, `clusterCourts.js`, `overpassCourts.js`) into a shared
`backend/src/utils/geo.js` while already touching two of those three files.

### 4. Postcodes on courts/clubs/areas (free — postcodes.io, no API key)

Explicitly chose the free/no-cost path over Google Geocoding (paid, would
need a billing setup) per Jack's direct call. New
`backend/src/utils/postcodeLookup.js`: a single best-effort lookup (used
inline on the two interactive write paths — dropping a court pin,
creating an area watch) and a bulk lookup (100 points/request, for a
one-off backfill). `postcode` column added to `courts`/`clubs`/
`area_watches`, format-checked at rest (a UK postcode regex — this app's
data is UK-focused). `clusterCourts.js`'s `reconcile()` also looks one up
for any club that doesn't have one yet, skipping the network call entirely
once it does (avoids re-querying postcodes.io on every re-cluster for
clubs that already resolved).

**Real backfill already run** against the local dev `app.db` (`node
scripts/backfillPostcodes.js`, confirmed with Jack first since it's ~333
real bulk HTTP requests): resolved 16,711 of 33,222 courts that predated
the postcode column (the rest have nothing postcodes.io could match — very
likely non-UK courts in the OSM seed, or genuinely remote ones). `verify:db`
stayed green afterward (94/94), which is itself a real validation that the
new postcode-format check is correct against real postcodes.io output at
scale, not just synthetic test rows.

### 5. Crowd-sourced club naming

`clusterCourts.js`'s auto-derived names (majority OSM tag, or a generic
"Courts near X" fallback) are often wrong or generic. Rather than pay for
an automated places/geocoding lookup, this reuses the exact pattern already
built and working for court-pin verification: `clubs.name_source`
('derived' | 'user'), `name_submitted_by`, `name_verified`, plus a new
`club_name_confirmations` table (same shape as `court_confirmations`).
`clusterCourts.js`'s `reconcile()` now leaves a `name_source = 'user'`
club's name alone on every future re-run instead of overwriting it with its
own guess (coordinates still get updated as courts join/leave). New routes
`POST /clubs/:id/name` (propose — resets confirmations for the *new* name;
a no-op resubmission of the already-current name doesn't reset progress)
and `POST /clubs/:id/name/confirm` (same `CONFIRMATION_THRESHOLD = 2` and
self-confirm block as `POST /courts/:id/confirm`). Frontend: `CourtSheet`'s
club row gained a postcode line, an "unverified name" banner mirroring the
existing unverified-court banner exactly, a suggest-a-name input, and a
confirm button.

### Not done this session (flagged, not started)

- Manual on-device verification of everything above — this sandbox can't
  run Expo/`react-native-maps`. Needs: drop an area-watch pin and confirm it
  saves/shows on the map; open My Watches and confirm all three sections
  list/remove correctly; confirm a previously-watched court now shows
...
## Session 2026-09-07 (later still³) — main-loop verification across all three layers

Jack asked to actually verify the core loop this session (the "later still²"
entry above only ran `compare_swing.py` in isolation once). Verified end-to-end
at **three layers**; no code changed.

### Results

- **Local Python engine** (`pro_matcher.py` → `compare_swing.compare()`) — 6/6
  cases exit 0, valid JSON: forehand (marked contact), backhand (marked), serve
  (marked + `--view-direction-hint back`), a 2nd forehand, forehand **no-mark
  auto-detect**, forehand **left-handed (`--handedness left`, mirror path)**.
  Every case produced pose → contact anchor → angle → candidate filter → DTW →
  3 matches + coaching tips + `overall_score`.
- **Local full stack** — started the Express backend on :5000, real
  `multipart POST /api/analyse` with a saved clip. HTTP 200 in ~28–55s, full
  payload including skeleton + racket overlay trajectories. `POST /api/history`
  → 201, `GET /api/history` → row present. Free-tier cap fired **403
  `DAILY_LIMIT` after 2 successful analyses** (works as designed). Response
  bytes verified clean UTF-8.
- **Live server** (`https://rallymax.167-233-107-31.sslip.io`) — `/health` OK,
  signup OK, forehand upload → HTTP 200 in 27s → top match `forehand_2039`
  sim 38.3 / `overall_score` 61.7 + tips + overlays. **Byte-identical score to
  the local run** → engine parity. Throwaway account deleted afterward (`DELETE
  /api/auth/me` → 204). The live candidate pool contained **no `practice_*`
  entries** where the local pool did → confirms the server is still on the
  older (pre-2026-09-02) pro DB, as STATUS item 14 says.

**The core loop works end to end, locally and in production.**

### New flaws found (added to STATUS "Right now" + TODO_MANUAL)

1. **Two different score scales are shown on the same `ResultsScreen`.** The
   hero score (`ResultsScreen.js:370`) is `top.overall_score ?? top.similarity`
   — `overall_score` is the sum of the 4 phase sub-scores (`phase_breakdown.py`,
   `PHASE_SCALE=1.8`) → lands **~62–70** for a competent swing. The 2nd/3rd
   matches (`ResultsScreen.js:581`) render raw `m.similarity` — the
   `similarity_score()` exp-decay with `scale=0.4` → lands **~25–38** for the
   *same quality* of match. So a user sees "62/100" as their headline then
   "other close matches: 30/100, 25/100" directly below. **Implication for the
   calibration task:** `JACK_TODO.md` frames it as "move `scale=0.4`", but
   `scale` only governs the runner-up numbers and the ≥75 "great swing" gate at
   `ResultsScreen.js:269` — the number the user reads as *their score* is
   `PHASE_SCALE`. Re-scope the task: either calibrate `PHASE_SCALE`, or put the
   top match and the runner-ups on one scale so they stop contradicting.
2. **Serve quality is visibly poor on real input.** Marked-contact serve
   scored `similarity` 18.2/100; top-3 serve matches were all reclassified
   practice-footage entries, not broadcast pros (~82 serve entries total).
   Reinforces weak-point #8 — a first user uploading a serve gets a bad-looking
   result.
3. **Camera-angle confidence is low on most real clips** (measured 0.05–0.64
   across the 6 cases, mostly ~0.3). When it's inconclusive the ±20° angle
   filter still leaves a large pool, so DTW silently runs against
   loosely-framed clips with no signal to the user. Reinforces weak-point #4.
4. **Auto contact detection** on a silent clip landed 1.90s vs 2.1s marked
   (~6 frames early) — consistent with the documented ~9f wrist-peak error;
   audio-onset correctly skipped (no audio track).

### Non-issues checked and cleared (so a later session doesn't re-chase them)

- `forehand_0039`-style entries appearing in a *backhand* candidate pool: the
  `id` string and `clip_path` filename are stale from an earlier build, but the
  `shot_type` field is correct (`backhand`) and both the DTW comparison and the
  displayed pro clip use it — consistent, just a misleading id.
- `practice_*` entries as top matches: all were `LABEL_REVIEW`-verdict'd in Pro
  Clip Review (637 reviewed ids), so `eligible_match_candidates()` correctly
  admits them. Their clip *filenames* still say `_forehand` where Jack
  relabelled them `serve`/`backhand` — cosmetic, the served clip is the
  reviewed one.
- Mojibake in coaching-tip text seen mid-session was the verification harness
  writing an intermediate file as cp1252 on Windows — `tip_selector.py` opens
  the DB with `encoding='utf-8'` and the real `/api/analyse` response is clean
  UTF-8. No app bug.

Leftover: 3 throwaway upload dirs under gitignored
`data/runtime/user_clips/upload_17888204*` from the full-stack test (the
`rm` was blocked by the sandbox classifier — harmless, gitignored).

---

## Session 2026-09-08 — ML-reliability roadmap for release (planning only, no code)

Jack asked to verify ML reliability across the whole app and build a roadmap to
launch. This session is **planning + documentation only** — nothing was coded.
Three Explore agents mapped the camera-angle/similarity paths, the shot-classifier
+ contact-detection code, and the verifier/swing-detector/racket/tips code; one
Plan agent designed the two hardest items. Full plan with appendices:
`C:\Users\jackp\.claude\plans\okay-so-i-finished-lovely-adleman.md`. The
followable checklist is also in `JACK_TODO.md` ("ML-reliability roadmap").

### Jack's decisions
- **Slip the release date.** The old ~2026-09-21 target is dropped — Jack wants
  every premium feature reliable at launch (App Store update friction), so the
  date moves to whenever the roadmap + plumbing are genuinely done. The
  "ML-reliability sprint tail is downstream of highlights, defer past launch"
  guidance in `STATUS.md`/`JACK_TODO.md` from 2026-09-07 is **superseded** — the
  highlights feature and its ML are now in scope for v1.
- **Behind-the-baseline camera view only for v1.** Reduces the camera-angle
  problem from "measure the angle precisely" to "gate: is this behind-baseline?
  reject clearly if not."
- **Invest in a visual contact-frame model** as the no-audio backstop (Phase 1c).
- **Serve classification (~80% broadcast / ~60% amateur) is acceptable for v1** —
  no dedicated serve sprint; the held-out practice serves flow into the pool as
  Jack reviews them.
- **Build a real highlights eval set** from Jack's own whole-match fence
  recordings (the existing 86.6%/68.1% numbers are broadcast-TV / YouTube-amateur
  — neither is the real use case).
- `sample_every` 3→1 (Jack runs the re-extract overnight); z-depth one more try.

### What the exploration established (facts, not opinions)
- **No behind-view classifier or gate exists** — only `detect_view_direction()`
  (`infer_angle.py:633`), a geometry heuristic validated on ~10 clips that just
  *abstains* on hard cases. `compare_swing.py:594-620` silently falls back to the
  whole shot-type pool when its angle/view filters yield <5 candidates — an
  unsupported view produces a meaningless DTW score with no user signal.
- **`similarity_score` is duplicated** (`compare_swing.py:95`, `phase_breakdown.py`),
  `scale=0.4` never calibrated, DTW is 2D-only over 9 landmarks. **No labelled
  match-quality set and no "Jack's clips" set exist** — both are new work.
- **The 86.6% shot-classifier number is broadcast TV**; 68.1% is YouTube amateur.
  Neither is phone-on-fence footage. **traj_knn serve recall 11% is not the live
  serve number** — the geom serve gate owns serves (ensemble serve 80%/60%);
  traj_knn is 11% only because its pool has 27 serve exemplars vs 233 FH / 153 BH.
- **`racket_tracker.find_contact_frame` never reaches production** (only via an
  inert trust gate). Live contact detection = geometric anchor + audio onset
  only. The "serve window widening in racket_tracker" code comment is **stale —
  no such widening exists**.
- **The serve-gate bug is already partially mitigated** — `POINT_BOUNDARY_GAP_SEC`
  is 12.0 and the default mode is `advisory` (keeps all non-serve shots). The
  "12/12 forehands discarded" no longer happens on the production path.
- **Racket keypoints (Pose mAP50 0.485) DO affect users** — the `body_rotation`
  phase sub-score uses them live — but are NOT in the contact/DTW scoring path.
- **Ball detector retrain** — finished + gated 2026-09-08, new `best.pt` KEPT
  (see "Session 2026-09-08 (later)" below). Not on the server yet.
- **Tip selector**: live path never calls Claude (`use_verifier=False`); the
  verifier is blocked on the leaked API key; the 3 top-match tip sets are
  computed independently and can contradict.

### Roadmap shape (see JACK_TODO.md for the checklist)
- **Phase 0** (now/overnight): `sample_every` 3→1 re-extract; finish + gate the
  ball retrain; Jack records fence footage (feeds 3 later items); Jack rotates
  the API key.
- **Phase 1** (core loop): behind-baseline camera gate + validation harness;
  similarity-score calibration (4-anchor method, doubles as a DTW-discrimination
  diagnostic); visual contact-frame model for the no-audio path; z-depth retry.
- **Phase 2** (premium): highlights eval on real fence footage (gates the rest);
  `occlusion_gap` verifier fix; swing-detector recall; racket-keypoint retrain;
  tip cross-match reconciliation + verifier loop.
- **Phase 3** (pre-release gate): full suite green, data+models to server, one
  real live upload, store plumbing.

### Not started
Everything above. No files changed this session except the plan file and these
docs.

## Session 2026-09-08 (later) — ball-detector retrain finished + gated: KEEP

Picked up JACK_TODO item **0b**. The imgsz-480 / multi_scale retrain from
2026-09-07 had stalled at epoch 107/150 (`results.csv` frozen at 23:23, no
process alive — laptop almost certainly slept). **Resumed from `last.pt`
(`resume=True`)** — the remaining 43 epochs ran in 0.75 h, all 150 complete.

- **Final model** (`data/10b_ball_detection/yolo_ball_run_v1/weights/best.pt`,
  epoch 126): clean-val **mAP50 0.592**, P 0.642 / R 0.658, mAP50-95 0.193.
  This is on the **leak-free** val split (262 train / 76 val, 0 image/clip
  overlap — the old 0.558 was on the leaky split, so not comparable). The
  extra 43 epochs barely moved it (epoch 105 was already 0.599) — the run
  just needed a clean finish.
- Trained in place (`exist_ok=True`), so `get_ball_model()` already loads the
  new weights locally. Pre-run backup kept: `yolo_ball_run_v1_BACKUP_20260907_165355`.

### The 3 gates (`README_ball_retrain.md`) — run 2026-09-08

No stored baseline JSON existed to diff against (audits are gitignored + get
overwritten), so compared against the numbers quoted in the README.

| Gate | Bar | Result | Call |
|---|---|---|---|
| `audit_finetuned_ball_confidence.py` at-contact | ≥ 93.3% | **91.7%** (55/60), avg conf **0.641** (was 0.557); generic COCO baseline 50.0% / 0.41 | ~flat, 1 clip under; confidence clearly up |
| `calibrate_ball_inference_scale.py` regime B (near-player crop) @ imgsz 320 | backhand up from ~0.77, FP flat | all **0.923**, FH **0.957**, **BH 0.769**, serve **0.942**, fp 0.121 | backhand unchanged; FH/serve strong |
| `eval_near_court_ball_detection.py` | near/far up | full-frame A@320: near **0.704**, far **0.792**; cropped near-court regimes B/C much worse (C far ~0-4%) | crop tracker already NO-GO; full-frame far-ball 79% |
| dataset leakage asserts | pass | pass (0 image + 0 clip overlap) | ✓ |

Regime A2 (near-court, higher res) @ imgsz 480: all 0.932, BH 0.829.

### Decision — KEEP (Jack's call, 2026-09-08)

Strict reading is "revert" (2 gates didn't clear), but the "backup" is the
older *leaky-trained* v1, and the new model has **higher confidence, an honest
non-leaky eval, and no real regression** (backhand is flat, not worse; FH/serve
near-player are ~94-96%). Jack reviewed the numbers and called it: **keep the
new `best.pt`, ball tracking is "good enough" for v1, close the retrain item.**

**Still not deployed** — `data/` isn't touched by CD, so the new `best.pt` is a
manual server transfer (tracked in TODO_MANUAL / JACK_TODO alongside the pro-DB
copy). Remaining known-weak: **backhand (~77%)** and **far/wide-court balls** —
both blocked on cleaning the ~8-of-22 sloppy `wide_court_ball_labels*.jsonl`
positives (held back 2026-09-07) + more amateur backhand footage. Logged as a
future-retrain item, not launch-blocking.

Files changed: docs only. Model weights already in place from the training run.

## Session 2026-09-08 (later²) — Roadmap 1c: visual (no-audio) contact-frame backstop

Plan: `C:\Users\jackp\.claude\plans\hashed-imagining-stardust.md`. Implements
roadmap item **1c** (`JACK_TODO.md` "ML-reliability roadmap"). Three parts.

**Part A — `find_contact_frame` wired into the live no-audio path.**
`compare_swing.compare()`'s auto-detect chain previously reached
`racket_tracker.find_contact_frame` only through the inert Phase C.4 block
(gated on `contact_frame_ml_training_log.stats()['trusted']`, which is False and
also carried a latent `TypeError` — `compute_contact_evidence` was called with a
`shot_type=` kwarg it doesn't accept). Replaced that block with a direct,
un-gated visual refinement: when there's no confident audio onset, call
`contact_evidence.compute_contact_evidence(...)` (already does
`track_racket_and_ball` + `find_contact_frame` + `contact_frame_meta` +
`wrist_kinematics`) and, if `student_method != 'wrist_velocity_fallback'`, use
`student_frame / fps` directly as the contact time. The twice-rejected Phase C
learned-offset model (`predict_contact_offset`) is **no longer referenced by the
live path** — `contact_frame_model.pkl` must not ship. Kept: the audio+visual
agreement shortcut (visual pick within 2f of a sub-threshold audio pick → take
the audio time), the try/except-and-carry-on guard, and the detached
`log_user_contact_frame_cli.py` training logger (now data-only, not a gate).
Consequence: the ~5s racket/ball YOLO pass now runs on every auto-detect upload
that isn't resolved by confident audio (previously it never ran in prod).

New `contact_evidence.compute_contact_evidence(..., search_window_sec=None)` —
threaded through to `find_contact_frame` / `contact_frame_meta`. Serves pass
`SERVE_CONTACT_REFINE_WINDOW_SEC = 0.45` (vs the 0.3s default) — the stale
"serve window widening in racket_tracker" comment finally made real, at the
call site rather than in `racket_tracker`.

**Part B — serve overhead-apex bimodal tail.** `serve_anchor.py`: the raw
`_argmax_earliest` tie-break lands on the *leading edge* of a wide apex plateau
on distant/foreshortened poses (one error mode near contact, one early). New
`_apex_plateau_frame()` finds the contiguous run within `APEX_PLATEAU_TOL = 0.04`
(torso-normalised reach units) of the max and returns the measurable frame
nearest that run's centre — a sharp apex (plateau width 1-2f) is unchanged; a
flat plateau is recentred off the early edge. `find_serve_apex_frame` now uses
it for both the `_above_head` and `_overhead_reach` passes. Options 2 (toss-arm
gate) and 3 (re-sweep `APEX_TO_CONTACT_LEAD_SEC`) from the plan are held pending
the eval numbers.

**Part C — audio-onset model observability.** `onset_classifier.pkl` (2.5 KB,
gitignored, hand-rsync'd to the server) had no way to confirm it's actually
deployed — the "Contact auto-detected via AUDIO onset" line is stderr-only and
`runPythonJson.js` discards stderr on a successful exit. Added:
(1) `result['contact_source']` in `compare_swing.compare()` — one of `user` /
`audio_onset` / `audio_visual` / `ball_occlusion_gap` / `ball_racket_proximity` /
`serve_apex` / `wrist_peak`. A no-mark upload returning `audio_onset` proves the
model is live; (2) an `onset_classifier` section in
`ml_status_report.py` → surfaces `model_present` via the existing
`GET /dev/ml-status`. `JACK_TODO.md` / `TODO_MANUAL.md` confirmation steps
updated to use these instead of the log grep that never worked.

**Eval harness fix (prereq for measuring B).** `eval_pro_clip_contact.py` gained
`--anchor auto` — routes each clip through `compare_swing.auto_contact_anchor_frame`
(so serves exercise the overhead-apex path) instead of `find_peak_wrist_frame`
directly. Writes a separate `_auto_anchor.csv`. `wrist` stays the default.

**Tests.** New `test_find_contact_frame_window_pytest.py` (restores the lost
window/precedence coverage — pure, no cv2). `test_serve_anchor_pytest.py`:
`test_plateau_returns_earliest_frame` → `test_plateau_returns_centre_frame` +
two more plateau cases. `test_contact_student_pytest.py`: serve-window
pass-through case. All 105 in `07_ball_racket_tracking/` + `00_utils/` green.
`eval_pro_clip_contact.py --anchor auto` baseline running at write time (~2.5h
on CPU); FH/BH rows must stay unchanged, serve median |err| must improve.
Re-run after Phase 0a (clear `.eval_pose_cache/` first). **Not committed.**

Files: `scripts/08_comparison_engine/compare_swing.py`,
`scripts/00_utils/serve_anchor.py`, `scripts/00_utils/ml_status_report.py`,
`scripts/07_ball_racket_tracking/{contact_evidence,eval_pro_clip_contact}.py`,
3 test files, `JACK_TODO.md`, `TODO_MANUAL.md`.


## Session 2026-09-08 (later³) — Roadmap 1b: the swing score can't be fixed by rescaling — rubric redesign started

**The question 1b was supposed to answer:** the `ResultsScreen` hero score
(`overall_score`, `PHASE_SCALE=1.8`) and the #2/#3 match scores (`similarity`,
`scale=0.4`) are on different uncalibrated scales — pick one, calibrate it so a
DB clip ≈ 100, a competent amateur ≈ 50, a fresh ATP swing ≈ high-80s.

**What the harness found (`scripts/17_amateur_eval/calibrate_similarity.py`, new).**
Three buckets from data already on disk — in-DB pro clips, held-out pro clips
(own entry excluded from the pool), 45 real labelled amateur clips
(`data/08_coaching_ai/amateur_swing_labels.json`). Best-match raw DTW distance,
median: **in-DB pro 0.15, held-out pro 0.45, amateur 0.48.** Held-out pro ≈
amateur — **no `scale` can produce Jack's target anchors**; at any scale where
amateur = 50, held-out pro ≈ 52. An in-DB clip re-run scores ~81 not 100 (the
user-side `extract_user_poses` pipeline ≠ the pro-DB build pipeline; ~0.09
same-clip noise floor).

**Round 2 — metric redesign attempts (`redesign_similarity.py`, new).** Cached
all 84 eval trajectories, benched ~10 DTW variants: Sakoe-Chiba band, warp
penalty, swing-arm landmark weighting, biomechanical-feature DTW, k-nearest-5,
tight angle windows. **Best separation +12%, still fully overlapping.**
Feature-DTW was *worse* (amateurs closer to pros than pros to each other).
Root cause is structural: nearest-neighbour distance against a 413-pro DB is
governed by DB density, not query quality.

**Jack's call:** reframe the number as match-tightness short-term; build a
**rule-based technique score** — grade a swing on independent biomechanical
axes against the pro DB's own distribution ("pros define the good range", no
new quality labels).

**Round 3–4 — the rubric works.** `redesign_similarity.py axes` (per-axis
pro-likeness, 100 = pro-DB median) + `rubric` (curated weighted combo). Added
**view-conditioning** — score each swing vs its own camera view's pro
distribution, since sign-dependent axes (racket-behind-body, wrist lateral)
are meaningless pooled across front/back views. Combined held-out-pro vs
amateur gap: **+3 (DTW) → +13 (rubric)**. Backhand **+32** (clean), serve
**+23** (view-conditioning rescued 5 axes that were noise), forehand **+10**
(still weakest). Single-swing noise still high (in-DB pros score 45–70, not
~90). Curated axes: contact wrist height, contact elbow angle, wrist-path
looseness ratio, tempo (backswing/through split), follow-through height,
backhand contact wrist lateral, serve tempo + wrist lateral + rotation range.

**Also fixed:** `scripts/00_utils/enrich_pro_racket_body.py` had a real bug —
it resolved `entry['clip_path']` (relative to `data/04_clips`) as an absolute
path, so every clip read as missing and it would have written all-null. Fixed
+ added resume/flush + a richer `racket_body_features()` (mean / extension
range / racket-path looseness) in `track_racket_in_clip.py`. **`racket_body_distance`
is on 0/648 pro entries** — meaning `phase_breakdown.score_body_rotation()`'s
racket half is also silently dead in production; the enrich run revives it.

**Queued on CPU (blocked by net-model training):** (1)
`python 00_utils/enrich_pro_racket_body.py` (~35 min); (2)
`python 17_amateur_eval/redesign_similarity.py cache` (grows held-out 18→58,
~30 min, checkpointed); (3) re-run `axes`/`rubric`, re-curate `CURATED_AXES`,
decide build-B2 vs accept-low-confidence-forehand/serve. **Runbook:
`scripts/17_amateur_eval/RESUME_swing_score_redesign.md`.** Full plan:
`~/.claude/plans/rosy-noodling-reef.md`.

**Nothing production / frontend / backend touched. Not committed.** Files:
`scripts/17_amateur_eval/{calibrate_similarity,redesign_similarity}.py` (new)
+ `RESUME_swing_score_redesign.md`, `scripts/00_utils/enrich_pro_racket_body.py`,
`scripts/07_ball_racket_tracking/track_racket_in_clip.py`, `STATUS.md`,
`JACK_TODO.md`.

---

## Session 2026-09-08 (later⁴) — Roadmap 1a: behind-the-baseline camera gate (advisory) + live-calibration alignment

**Goal:** v1 supports only the behind-the-baseline camera view, but today an
unsupported view (filmed from the net, or wildly side-on) is scored silently
against the whole pro pool — `compare_swing.py` computes `user_angle` /
`angle_conf` / `user_view_direction` and branches on none of them. Jack's call:
**advisory warning first** (1a-val threshold tuning isn't done), signal =
**view_direction + angle**, and align the live "Record now" calibration so it
warns *before* the user records rather than after they see their result.

**Shared verdict — `scripts/05_angle_detection/infer_angle.py`:**
`evaluate_view_usable(view_direction, angle_deg, angle_conf) -> {usable, reason,
severity, message}`. First failing rule wins:
- `view_direction == 'front'` → `front_view`
- `angle_deg >= VIEW_GATE_SIDE_ON_ANGLE_DEG` (78°, ≈ angle_label "Side view") → `side_on`
- `angle_conf < VIEW_GATE_MIN_ANGLE_CONF` (0.25; real clips sit ~0.3) → `angle_unreliable`
- else usable. `view_direction == 'unknown'` alone does **not** fail (detection is
  unreliable; the record-time picker hint is the tie-breaker).
Copy lives in `VIEW_GATE_MESSAGES` (full) / `VIEW_GATE_LIVE_MESSAGES` (badge).

**Wiring:**
- `compare_swing.compare()` — calls the gate after `user_view_direction` resolves,
  adds `result['view_gate']`, prints the verdict to stderr. **DTW still runs.**
  Hard reject only when `RALLYMAX_ENFORCE_VIEW_GATE=1` (env) — then it raises and
  `analyse.js`'s `nonzero_exit` branch surfaces the message. Default off.
- `check_camera_setup_frame()` (live badge path) + `check_camera_setup.py` (post-pick
  video path) — same gate; `ok:False` + reason message when not usable, so the
  badge goes red / the banner warns via the existing frontend branching with **no
  frontend change to those screens**. `calibration_server.py` untouched (forwards
  the dict). `check_camera_setup.py` samples one mid-video frame for
  `detect_view_direction` (new `cv2` import + `_detect_view_direction_for_video`).
- `frontend/screens/ResultsScreen.js` — amber warning banner when
  `result.view_gate.usable === false`, above the score card; results still render.
- `backend/src/routes/analyse.js` — comment only (points at the env switch).

**Tests:** 7 new pure-function cases in `test_infer_angle_pytest.py` (16 pass).
Full `pytest` 316 pass; `backend` `npm test` 620 pass.

**Open / not done here:**
- **1a-val** — hand-label ~60 pro + ~40 amateur + Jack's 0c clips, tune `VIEW_GATE_*`
  to <5% false-reject, then set `RALLYMAX_ENFORCE_VIEW_GATE=1` on the server.
- **On-device** — the outstanding `TODO_MANUAL.md` live-calibration click-through
  (badge red from the net / green from behind the baseline; warning banner on a
  deliberately wrong-angle upload), plus confirm the hosted `calibration_server`
  is up on :5055 with `data/10_net_detection/yolo_pose_run_v4/weights/best.pt`
  present (past crash-loop cause).

**Files:** `scripts/05_angle_detection/infer_angle.py`,
`scripts/05_angle_detection/test_infer_angle_pytest.py`,
`scripts/08_comparison_engine/compare_swing.py`,
`scripts/00_utils/check_camera_setup.py`, `backend/src/routes/analyse.js`,
`frontend/screens/ResultsScreen.js`, `STATUS.md`, `JACK_TODO.md`. Not committed.

---

## Session 2026-09-08 (later⁵) — score presentation: drop the pro identity, remove the contradicting #2/#3 numbers

**Problem (JACK_TODO "Score presentation + calibration", problem 1):** `ResultsScreen`
showed the hero score as `overall_score` (`phase_breakdown.PHASE_SCALE=1.8`, ~62–70
for a decent swing) and then an "Other close matches" list right below with raw
`similarity` (`compare_swing.similarity_score`, `scale=0.4`, ~25–38 for the same
quality) — two different uncalibrated scales, visibly contradicting each other.

**Jack's call:** don't reconcile the two numbers — **remove the pro-match framing
entirely.** Most `pro_database.json` clips were never identified
(`player_names.json` is partial), so `formatProId` fell back to "Forehand
Technique #142", which reads as broken. The result is a **technique score graded
against the pro database as a whole**, not a match to one named player.

**Changes (frontend, all user-facing spots):**
- `ResultsScreen.js` — "Other close matches" section **deleted** (this removes the
  `scale=0.4` numbers from the UI). `formatProId` replaced with
  `proMatchCaption(shotType)` → score-card caption "How closely your <shot>
  matches pro technique". Share-card caption → "Pro technique match". Sync Compare
  `labelA` → "Pro swing". Dead `otherMatches` / `scoreColor` import / `otherCard`
  styles removed.
- `HistoryScreen.js` — row no longer prints `formatProId` (shot type is already in
  the card header); score label "Match score" → "Technique score"; delete-confirm
  text de-named; `labelA` → "Pro swing"; `formatProId` + `proId` style removed.
- `CoachScreen.js` / `HomeScreen.js` — `formatProId` / `item.pro_id` row lines
  removed.
- `FirstSwingCard.js`, `SignupScreen.js`, `HistoryScreen.js` empty state — "matched
  to a pro" marketing copy → "scored against pro technique".
- `VersusResultsScreen` untouched (1v1 "reference video" is a clip the user
  themselves picked).

**Backend:** `analyse.js` now passes `--top 1` (was `--top 3`) — nothing consumes
`matches[1..]` any more, and it skips 2× `get_coaching_tips` per analysis in
`compare_swing.py`. `pro_id` / `player_name` still ride along in the payload
(needed for `pro_clip_url` resolution, history storage, dev tools) — just not
displayed.

**Not done:** `PHASE_SCALE` is still uncalibrated against a labelled match-quality
set (JACK_TODO problem 2) — that's roadmap **1b**, still CPU-blocked. `scale=0.4`
now only governs the ≥75 "great swing" gate and the `similarity` fallback stored
when phase breakdown fails.

**Tests:** `backend` `npm test` 620 pass. Frontend has no runner; all 7 touched
files babel-parse clean. Not committed. Files: `frontend/screens/{ResultsScreen,
HistoryScreen,CoachScreen,HomeScreen,SignupScreen}.js`,
`frontend/components/{FirstSwingCard,ResultShareCard}.js` (caption prop only),
`backend/src/routes/analyse.js`, `STATUS.md`, `JACK_TODO.md`.

## Session 2026-09-09 — camera-angle: normalization abandoned, view gate ENFORCED for launch

**Context.** Two attempts this week to make a user's swing geometrically
comparable to pros regardless of camera angle, both **validated against 6
known-angle clips Jack filmed** (`data/05_angle_detection/yaw_calib/`, one self-fed
forehand from ~0/18/30/30/45/88°) and both **failed**:
- **Pose-based yaw** (`viewpoint_normalization.py`, committed `dcc6500`/`2355594`/
  `61bd098`, flag `RALLYMAX_YAW_NORM` OFF, inert): reads body-facing azimuth from
  MediaPipe `pose_world_landmarks` lead-in, rotates the swing to canonical facing.
  Spearman(azimuth, camera_angle) = **-0.09** (target 0.8). Monocular 3D depth too
  noisy at ~10 m; self-fed footage has no stable square lead-in. Kept committed
  and inert — do NOT promote.
- **Net-width un-foreshortening** (Option B): `acos(net_width / FULL_NET_FRACTION)`
  → un-stretch the trajectory. Net apparent width 0.56/0.50/0.53/0.51/0.46 for
  0/18/30/30/45° — no monotonic signal. Zoom-confounded (whole net fills only
  0.555 of frame head-on, not 0.80), far post leaves frame on angled shots,
  model abstains at side-on. `FULL_NET_FRACTION` stays 0.80.

Full write-up: `~/.claude/plans/purrfect-jingling-rose.md`.

**Jack's call:** stop trying to *correct* camera angle. **Restrict the input
domain** — only accept clean behind-the-baseline footage with the whole net in
frame. In that band residual foreshortening (<6% at 20°) is below the pipeline
noise floor. This turns roadmap **1a** from advisory to enforced.

**Changes:**
- `infer_angle.py` — `_aggregate_frame_angles` adds `posts_inframe_frac` to debug
  (fraction of trained-keypoint frames with both net posts inside
  `NET_POST_EDGE_MARGIN`=0.03 of the frame edge; left/right x reconstructed from
  `net_center_x ± net_width/2`). `evaluate_view_usable(..., net_debug=None)` gains
  two block reasons: `net_not_found` (`net_detection_method != 'keypoint_model'`)
  and `net_truncated` (`posts_inframe_frac < POSTS_INFRAME_MIN_FRAC`=0.5). Return
  now carries `severity` ∈ `ok|warn|block`; `VIEW_GATE_BLOCK_REASONS` =
  {front_view, side_on, net_not_found, net_truncated}. `angle_unreliable` (conf
  <0.45, net found) is `warn` — false-rejects otherwise-fine footage.
- `compare_swing.py` — new `ViewGateError(RuntimeError)` with `.code`. Gate is
  enforced **by default**: `severity=='block'` raises `ViewGateError(code=
  'VIEW_NOT_USABLE')` unless `RALLYMAX_ENFORCE_VIEW_GATE=0` (advisory, for batch
  eval over legacy footage). Passes `net_debug=angle_debug` (or a
  `{'net_detection_method': None}` stub when the net wasn't found and `angle_debug`
  is a string).
- `check_camera_setup.py` + `check_camera_setup_frame()` — pass `net_debug`, add
  `view_severity` to every return.
- `pro_matcher.py` — error payload gains `code` from `getattr(e, 'code', None)`.
- `analyse.js` — `nonzero_exit` branch parses `{error, code}` and forwards `code`
  in the 500 body.
- `ResultsScreen.js` — `errorCode === 'VIEW_NOT_USABLE'` → "Check your camera
  setup" title + "Record another swing" (→ `popToTop`), no "Try again".
- `ContactMarkingScreen.js` — `calibrationBlocked` (`view_severity === 'block'`)
  → ⛔ red message in the rough phase, confirm button becomes "Record another
  swing" and `confirmFrame` short-circuits to `popToTop`.

**Verified:** `infer_camera_angle` on the 6 clips → 5901/02/05/06 `severity=ok`,
5907 + 5909 `block` (net_not_found). `compare_swing.py IMG_5907.MOV` raises
`ViewGateError`; `RALLYMAX_ENFORCE_VIEW_GATE=0` lets it score.

**Tests:** `test_infer_angle_pytest.py` +8 cases (29 pass). Full `pytest` 348
pass. New `backend/src/routes/analyse.viewGate.test.js`. Not committed yet.

## Session 2026-09-09/10 — metric-3D v4 pro-DB rebuild (stride-1 + world-z on every entry) + shot-contact verifier investigation

Two threads across several parallel sessions. Commits `6cfee83` → `4d19e2b` +
`12cab96`/`13771e7`, **local, NOT pushed** (user-side stride-1 must not reach the
server before the rebuilt pro DB is transferred there).

### Thread 1 — the pro-DB rebuild

**`sample_every` 3 → 1 (roadmap 0a done).** `extract_poses` default + its 3
callers (`process_new_videos`, `ingest_practice_footage`, `test_new_footage`) +
`compare_swing.extract_user_poses` (dropped the `idx % 3` gate) all keep every
frame now, and `extract_poses` always emits MediaPipe `pose_world_landmarks`. New
`scripts/02_pose_extraction/reextract_pro_poses.py` re-extracted all 13 pro
source videos (9 broadcast compilations + 4 practice) at stride-1 + world
landmarks — **~4.75 h, 13/13, 0 failures**; pose files ~6× bigger
(`backhand_poses.json` 114 MB → 713 MB, ~3 GB total). Writes a
`.stride1_reextract_complete` sentinel; old files → `_backup_pre_stride1_<ts>/`.

**Pro-side yaw wiring.** `trajectory_extraction.extract_swing_trajectory` gained
keyword-only `world_pose_index` / `yaw_enabled` (default off = byte-identical) —
reads camera azimuth from the swing lead-in, rotates to canonical facing (same
machinery the user side already had behind `RALLYMAX_YAW_NORM`).
`rebuild_helpers._load_pose_bundle` returns `(fps, pose_index, world_pose_index)`,
lru-cached; `reextract_for_entry` threads it through.

**v3 → v4 re-slice.** New `scripts/06_database_build/reslice_all_trajectories.py`
re-slices every one of the 648 entries' `trajectory` + `overlay` around its
existing `clip_contact_time_sec` (video-free / MediaPipe-free — preserves
`camera_angle` / `racket_body_*` / `view_direction` / practice-holdout metadata).
v3 first: density 24 → 73 pts/entry (3.1×). Then **v4** (`be1e1c4`): the z channel
is **decoupled from x/y** — x/y stays image (or hard-yaw-rotated), z is always
metric world-landmark-derived, rotated by a "soft yaw" (drops only the deadband
check) or left unrotated-but-metric. Schema `traj_version` 4, per-entry
`traj_yaw_deg` / `traj_z_yaw_deg`. Result: **648/648 v4, 388 z-rotated / 260
unrotated-metric, `verify:db` 99/99**, DB-wide abs-median z 0.45 (was ~1.5 on the
un-rotated half). Backup `pro_database_backup_pre_stride1_reslice_20260910_003017.json`.
Serve re-anchor (`reanchor_pro_serves.py`, now `yaw_enabled=True`) moved 1 serve.

**The one live change: `phase_breakdown.Z_ROTATION_BLEND = 0.0`.**
`Z_TO_DEG_SCALE`=28.6 was fit to image-z; every entry's z scale shifted with v4,
so the blend would mis-scale the live `body_rotation` sub-score. Reverts it to
angle-only (months-stable). Reversible — Phase 4 = re-derive on metric z.

**Orchestrator** `scripts/17_amateur_eval/run_metric3d_rebuild.py` — waits for
the sentinel, then reslice → reanchor → verify:db → amateur-eval →
`redesign_similarity` cache/axes/rubric, tee'd to
`data/17_amateur_eval/metric3d_rebuild_<ts>.log`. Stops before any `CURATED_AXES`
promotion.

**Result — DTW:** `redesign_similarity eval` v0 separation **+7% → +11%**
(held-out pro median held at 0.443, amateur moved out 0.477 → 0.498).

**Result — depth axes: NEGATIVE on real footage.** Bench (broadcast/YouTube):
`contact_depth_ahead` sep FH +30 / BH +17 / serve +18; serve depth axes cleared
the ≥15-sample floor for the first time (back-n 0 → 18), backhand firmed up
(n 19 → 48, +0 → +17). `redesign_similarity rubric` combined pro-vs-amateur gap
+7.8 → **+20.0** with a mechanically re-curated `CURATED_AXES` (`4d19e2b`, keep
sep ≥ +10, weight sep/10 — **flagged PROVISIONAL**). **But** on Jack's 6
known-angle fence clips, `contact_depth_ahead` ranged **[−0.19, +4.27] for the
same self-fed forehand at the same 0° camera position** — pose noise dominates,
no technique signal (same broadcast-vs-phone domain gap as the shot classifier).
**Decision: 3D depth as a scoring signal is done (negative result).** Pull the
depth axes from `CURATED_AXES` before the rubric drives anything real; roadmap
1b's foundation is the strong 2D axes (`tempo_peak_frac` +62,
`racket_body_range` +51, `backswing_depth` +37, contact wrist position,
follow-through). The metric-z *scale* fix (unit conversion) is real and kept; the
*scoring* use is not.

**"Calibrate camera angle once per session" (Jack, 2026-09-10) — tested RED.**
New `scripts/05_angle_detection/calibration_hold_test.py` (`12cab96`/`13771e7`):
camera→net angle is a session constant, so read it once from a deliberate
~2-3 s square-and-still hold, average `facing_azimuth` over 30-60 frames to beat
the depth noise, reuse for every swing. Run on the 6 existing yaw_calib clips
(self-fed, a lenient proxy): **within-hold facing IQR median 59°** (facing
bounces ±30° while standing still — a deliberate hold doesn't fix *that*), head-on
clip reads −57°, Spearman(|az|,|true|) +0.31, net-model also RED (−0.43).
**Third independent confirmation** (per-clip pose-yaw, net-width, now this) that
pose/net camera-angle normalization fails at fence distance. A
deliberate-square-hold film from Jack would close it 100% but RED is expected.
Fall back to the view gate + rubric front/back bucketing.

### Thread 2 — shot-contact verifier investigation (uncommitted)

The rebuild's amateur-eval gate flagged the verifier at 54.4% vs a "64.5%
baseline". **Investigated (2 Explore passes + a ball-model A/B): nothing
regressed.**
- `data/17_amateur_eval/results.jsonl` is an append-only, gitignored checkpoint
  grown 116 → 248 rows over ~a month, spanning the `b91abf5` batch merge
  (2026-09-07) that introduced the ML verifier + ball-tracker rework. The "64.5%"
  rows ran under materially older code and a lighter `skip`-label mix.
- The verifier reads a **frozen-since-August** `data/17_amateur_eval/poses/`
  cache (`sample_every=3`, hardcoded), never calls `extract_user_poses`, never
  reads the pro DB. `git diff 4486290..HEAD` on `scripts/16_shot_verification/` +
  `scripts/07_ball_racket_tracking/` = one README line.
- Regenerating the frozen pose cache with current MediaPipe: **0 pt change**.
- Old ball model (2026-09-06 backup) vs current: current is **better** (54.4% vs
  50.0%) — the 2026-09-08 retrain helps the verifier.
- The classifier (49.1%) is flat everywhere — the documented rule-based ceiling.
- **Neither the verifier nor the shot classifier runs on a real upload.**
  `analyse.js` → `pro_matcher.py` → `compare_swing.compare()` only. They're
  offline tools (rally highlights, pro-DB ingest, trust-log training).

**The real issue is absolute.** Verifier precision ~51% because `occlusion_gap`
(151/248 clips, ~49% agreement) keeps nearly every clip as "real" (recall 89%).
`_find_gap_contact` treats any ≥2-frame gap between ball detections as contact
evidence, and `filter_verified_swings` keeps any `occlusion_gap`/`proximity`
verdict regardless of confidence — so a better ball detector produces more
spurious "gaps" on non-shots. This is JACK_TODO **2b**.

**Eval hygiene shipped (Track 1, uncommitted):**
- `evaluate_amateur_dataset.py` — every result row stamped with `git_sha` +
  ball-model hash + `HARNESS_VERSION`; new `--fresh` flag (backup-then-clear);
  `print_report` warns when rows span multiple versions. Kills the
  stale-mixed-checkpoint failure class for every future eval in the repo.
- `racket_tracker.py` — `RALLYMAX_BALL_MODEL` env override on `BALL_MODEL_PATH`.
- Clean stamped baseline: **`data/17_amateur_eval/results_baseline_2b27847.jsonl`**
  (54.4% verifier / 49.1% classifier). Frozen-Aug cache → `_poses_backup_frozen_aug_<ts>/`.

**Track 2 (the `occlusion_gap` re-calibration) is next session** —
anchor-proximity constraint + confidence gate + threshold sweep, ~1-2 days. Plan:
`~/.claude/plans/rustling-meandering-petal.md`. Not launch-blocking.

**Uncommitted after this session:** verifier eval-hygiene files (`racket_tracker.py`,
`evaluate_amateur_dataset.py`, `reanchor_pro_serves.py` + its test) — land with
the Track 2 batch.

## Session 2026-09-09/10 — pre-release check: security hardening + guest onboarding (Option B) + rating prompt

A separate thread from the ML work above, driven by four external videos (ASO +
"vibe-code security"). Full running checklist: **`PRE_RELEASE_CHECK.md`** (repo
root, new). Onboarding design: **`docs/plans/onboarding_plan.md`** (new).
**Everything below is uncommitted** and tangled in the working tree with Jack's
own in-flight WIP on `analyse.js` / `ResultsScreen.js` / `SignupScreen.js` /
`TODO_MANUAL.md` — committing needs a hunk-split or a combined commit, Jack's
call (not yet made).

**Security (backend was already heavily hardened — most of A1/A2/A5/A6/A7 pre-done):**
- IP-keyed rate-limit backstop on `/analyse` + `/compare-videos` (`analyseIpLimiter`
  80/10min, `compareIpLimiter` 40/10min) behind the existing per-user limiters —
  closes the account-rotation gap (`PRE_RELEASE_CHECK.md` A1).
- New `analysis_usage.daily_cap` integrity check; `FREE_TIER_DAILY_ANALYSIS_LIMIT`
  moved out of two route files into `domain/invariants.js`. `verify:db` 98→99.
- New `middleware/securityHeaders.js` (nosniff / X-Frame-Options DENY /
  Referrer-Policy no-referrer / CORP cross-origin / CSP on the served
  reset-password.html only) — hand-rolled, no `helmet` dep.
- `multer` 2.2.0 → 2.3.0 (4 high-severity upload-path DoS advisories). Removed
  unused deps `pg` / `redis` / `axios`.
- IDOR / broken-authz sweep of every `:id` route — all enforce ownership, no change.
- Deferred (need Jack, all `[infra]`): budget caps + alerts on KIE / Anthropic /
  AWS / Resend / RevenueCat and scoping the AWS IAM key to S3-only (**the single
  most important open pre-launch item**); securityheaders.com scan post-deploy;
  `js-yaml` (dev-only) + 35 Expo-toolchain `npm audit` advisories (do NOT
  `audit fix --force` on SDK 54).

**Onboarding — Phase 1, "gate at the reveal" (Option B):**
- `/analyse` `requireAuth` → `optionalAuth`. A logged-out user can run **one**
  analysis so the flow can show the score before asking for a signup. `isGuest`
  skips the free-tier reservation, handedness lookup, and contact-frame logger.
- Guest cap: **2 / 24h per IP**, consumed via `tryConsume()` *inside* the handler
  after every validation gate (so a bad file pick costs nothing), 429 carries
  `code: 'GUEST_LIMIT'`. Residual gap (a capped free user drops their token for
  2 more runs/IP) documented, not closed — not worth engineering against on one box.
- Frontend: new `OnboardingScreen` (one screen — payoff line + 3 steps + CTA),
  `utils/onboarding.js` (`onboarding_complete` gate), `App.js` `RootNavigator`
  picks the initial route (Onboarding only when not-complete AND not-authed).
- `ResultsScreen`: **fixed a latent bug — `runAnalysis()` never sent the auth
  header**, so every authed analysis was going through unauthenticated. Plus a
  guest "reveal gate" (score held behind "create a free account"), and an effect
  that saves the held result once `isAuthenticated` flips.
- `Signup`/`Login` take a `returnTo` param (shared `utils/navigateAfterAuth.js`)
  so signup mid-flow pops back to the result, not Home.
- Not built (Phase 2, deferred): result coach-marks, contact-frame lead-in card,
  permission pre-prompts, welcome-email drip, funnel instrumentation.

**Rating prompt (ASO — `PRE_RELEASE_CHECK.md` B3):** `utils/reviewPrompt.js` +
`expo-store-review@~9.0.9`. `recordHappyEvent()` fires immediately on a saved
fresh analysis; `considerReviewPrompt()` (2.2s later) shows the native sheet only
if gated conditions pass (authed, score ≥ 50, 2nd+ event, ≥ 60d apart, ≤ 3
lifetime, OS has the final say). Dev Page has a "Test 'rate the app' prompt" button.

**Review pass:** `/code-review` (Standards + Spec parallel sub-agents) + a
correctness pass over the uncommitted diff → 8 fixes applied (guest cap moved
inline + 3→2, `GUEST_LIMIT` code, stale comments, dead `saveStatus === 'guest'`
banner removed, `ipRateLimit` + `navigateAfterAuth` helpers extracted,
`recordHappyEvent` split out). Backend suite **631 green**, `verify:db` 99/99,
web bundle builds clean.

**Manual follow-ups → `TODO_MANUAL.md`** (2026-09-09 section): device-test the
onboarding + rating prompt (guest analysis needs the `optionalAuth` change
*deployed*); add `ios.appStoreUrl` / `android.playStoreUrl` to `app.json` once
listings exist; ASO Section B (store name, keyword research, screenshots);
onboarding Phase 2 scope.

## Session 2026-09-10 (later) — ideal-swing reference clips DONE (Luma Modify route) + pre-release roadmap consolidation

**Reference clips shipped.** The display-only side-by-side "ideal swing" clips
(FH/BH/serve) — dormant frontend scaffold since 2026-09-08 — are produced and
wired in. **Uncommitted** (the file + assets dir were never committed).

- **Route that worked:** Luma Dream Machine **"Modify Video"** (lumalabs.ai
  directly — *not* the third-party reseller apps, which are dark-pattern
  paywalls). Feed a real behind-view swing clip + a character start-frame; Luma
  re-renders replacing the person while keeping the real motion. Jack ran Luma
  himself (3000 free trial credits, 540p). All earlier routes had failed:
  Mixamo (no tennis anims), monocular mocap (a groundstroke travels along the
  camera axis — unmeasurable), hand-keyed IK mannequin (stiff), CapCut
  compositing (flickery).
- **Source clips** (behind-view, from `data/04_clips/`, verified frame-by-frame
  — the DB `view_direction`/`shot_type` fields are unreliable): forehand
  `forehand_0209`, backhand `backhand_0246`, serve `serve_0176`. Claude
  transcoded them to clean H.264 for Luma upload via a new
  `rallymax_reference_wip/blender/transcode.py` (Blender VSE + its bundled
  ffmpeg — there is no standalone ffmpeg on this machine and cv2's H.264 writer
  is broken).
- **Post (Claude, `rallymax_reference_wip/blender/make_clean.py` + `make_src.py`
  + `finalize.py`):** Luma stamps two watermarks — its own "Luma AI" logo
  (which *drifts* top-left → top-right partway through each clip) and the
  garbled "Court Level Tennis" YouTube mark bottom-right. Removed both. Then
  **Jack cropped around the player in CapCut** (his call over Claude's blur
  approach — a tighter crop removes the top watermark strip too) and did light
  retiming. Claude took the CapCut outputs (2560×1440), trimmed the letterbox
  bars, downscaled to 720p H.264.
- **Delivered:** `frontend/assets/reference/{forehand,backhand,serve}-ideal.mp4`
  (587 KB / 1.4 MB / 928 KB; content ARs 1.38 / 1.27 / 1.15 — `SyncCompareScreen`
  renders CONTAIN so per-clip AR mismatch is fine). Kit isn't perfectly matched
  (FH/serve white shorts, BH green) — Jack accepted it; the clips never play
  side-by-side with each other. **No synthetic ball** — Luma's ball reads OK
  through contact on all three.
- **Wiring:** `frontend/config/referenceClips.js` — 3 `require()`s uncommented,
  `CONTACT_SEC = {forehand: 0.43, backhand: 1.0, serve: 0.3}` (Claude eyeball
  estimates; **Jack to refine that one line** with measured values). No other
  code change — `ResultsScreen` / `TipsSection` already consume the hook.
  `npx expo export --platform web` succeeds, all 3 mp4s bundle with content
  hashes. WIP + scripts stay in `C:\Users\jackp\Documents\rallymax_reference_wip\`.

**Pre-release roadmap consolidation.** Jack asked "what's left / can we start app
testing?". A consolidated read of `PRE_RELEASE_CHECK.md` + `JACK_TODO.md` +
`STATUS.md` + `okay-so-i-finished-lovely-adleman.md` + git state is in
**`C:\Users\jackp\.claude\plans\snazzy-jumping-rain.md`**. Headlines:

- **`master` is 22 commits ahead of `origin/master`, not pushed** — and must
  stay unpushed until **JACK_TODO 3b** (server `data/` transfer: rebuilt
  `pro_database.json` + overlays + `onset_classifier.pkl` + ball `best.pt`, then
  `clusterCourts.js` on the box). Pushing first auto-deploys the user-side
  stride-1 change against the server's stride-3 pro DB → score drift. **This is
  the #1 critical-path item; app testing against live is testing stale ML until
  it's done.**
- **~41 modified + ~28 untracked files uncommitted**, tangling 5 threads
  (security, onboarding, rating prompt, v4 follow-ups, reference clips). Commit
  strategy is Jack's call.
- **Roadmap 1b + 1c are both blocked on a Jack decision, not Claude work:**
  - **1c** — the serve accuracy eval was already run (2026-09-08): before/after
    the apex-recentring change is **identical** (no regression, no improvement;
    gate passes trivially). Real serve error = low-camera-angle occlusion-gap
    blowups, outside 1c scope. Decision: (a) ship 1c as safe-but-neutral + file
    the occlusion-gap problem separately, or (b) tune `APEX_PLATEAU_TOL` first.
    Also: only the `serve_anchor.py` + `racket_tracker.py` changes are in the
    working tree — the claimed `compare()` no-audio wiring is **not there**
    (lost in the uncommitted churn, needs redoing before 1c ships).
    `scripts/07_ball_racket_tracking/RESUME_1c_contact_eval.md`.
  - **1b** — rubric bench hit **+20** pro-vs-amateur gap on the v4 DB but it's
    PROVISIONAL: forehand axis still weak (+5.8, not backhand-grade),
    `contact_depth_ahead` (a depth axis that's **noise** on Jack's fence
    footage) is still weighted in `CURATED_AXES`, and it's **bench-only, not
    wired into production**. Pulling the depth axes + re-running needs the v4
    traj-cache rebuilt (CPU). Bigger call: ship forehand `confidence: 'low'` or
    build new forehand axes first.
    `scripts/17_amateur_eval/RESUME_swing_score_redesign.md`.

---

## Session 2026-09-10 (later²) — working tree committed (three-session split)

The ~40-file uncommitted tangle on `master` was committed, split across the
three Claude sessions live in the repo at the time:

- **tennis-app-ea** committed roadmap 1c contact detection — `2791e7e` (serve
  apex-plateau anchor + onset-classifier observability) + `f28f3b5` (contact
  eval harness, stride-1). 8 files.
- **tennis-app-0f** kept roadmap 1b (`scripts/17_amateur_eval/redesign_similarity.py`,
  `evaluate_amateur_dataset.py`, and the not-yet-created `compare_swing.py` /
  `phase_breakdown.py` / `history.js` / `PhaseBreakdown.js` edits) **uncommitted**
  — that slice lands on its own branch later.
- **this session** committed everything else, as:
  1. `net-detection: v6 group-split + v10 keypoint retrain + testset v1` — the
     `scripts/10_net_detection/` training/eval tooling (offline).
  2. `pro racket enrichment` — portable+resumable `enrich_pro_racket_body.py`,
     richer `racket_body_features()` (feeds 0f's rubric, no score change yet).
  3. `serve reanchor: yaw-parity` — `reanchor_pro_serves.py` `yaw_enabled=True`;
     `RALLYMAX_BALL_MODEL` env override in `racket_tracker.py`.
  4. `pre-release backend: security hardening + guest analysis path` — IP-keyed
     rate-limit backstop, `securityHeaders.js`, `analysis_usage.daily_cap`
     invariant, multer 2.2→2.3, drop `pg`/`redis`/`axios`, `/analyse`
     `optionalAuth` + 2/24h guest IP cap, `--top 1`. Suite 631 green,
     `verify:db` 99/99.
  5. `score presentation: drop pro-identity framing` — History/Coach/Home cards.
  6. `reference clips` — bundled FH/BH/serve ideal-swing mp4s (~2.8 MB) +
     `referenceClips.js` + per-tip "See this done right" + SyncCompare
     `focusNote`/`initialT`.
  7. `ASO: native review prompt` — `expo-store-review`, `reviewPrompt.js`, dev
     test button.
  8. `onboarding Phase 1: guest "gate at the reveal"` — `OnboardingScreen`,
     `App.js` `RootNavigator`, `navigateAfterAuth.js`, the `ResultsScreen`
     reveal-gate + **the latent missing-`Authorization`-header fix on
     `runAnalysis()`**, plus the `ResultsScreen` wiring for commits 5–7 and the
     roadmap-1a view-gate banner.
  9. `docs` — this entry + STATUS/JACK_TODO/TODO_MANUAL.

**Not pushed** — `master` is ~34 commits ahead of `origin/master` and stays that
way until JACK_TODO 3b (the server `data/` transfer). A push auto-deploys and
would run stride-1/v4 user trajectories against the server's stride-3/v3 pro DB
→ wrong scores (no crash, no data loss, ~10 pts of drift, reversible). Nothing
