...
## Scheduled-routine PR round-up (2026-09-07)

Checked via the GitHub MCP tools (`search_pull_requests`/`list_pull_requests`,
no `gh` CLI in this environment) for PRs created in the last 24 hours. Five
routines fired overnight — **PRs #32–#36, all still open**, none merged
(these routines never merge their own work). **No `🚨 CRITICAL:` PR
today** — the security review explicitly states no CRITICAL finding.
Today is a Monday, so the weekly-cadence brainstorm and training-data-
drift-watch routines both fired alongside the every-3-days logic/bug/
security trio (last fired 2026-09-04) — a full 5-routine day, not a
coincidence.

- **PR #32 — "Training-data drift watch 2026-09-07: no data reachable,
  routine structurally blocked"** (branch `training-drift-watch/2026-09-07`,
  opened 03:08 UTC). Same structural gap on this routine's first-ever run
  since it was added 2026-08-26: `data/` is gitignored and this routine
  only ever gets a fresh `git clone`, with no SSH/infra access to where the
  training-log `.jsonl` files actually accumulate from live traffic, so
  there is nothing to check for drift. No code changes; adds a dated
  section to `TODO_MANUAL.md` writing up three options for closing the gap
  (sync the 4 small jsonl files out from behind `.gitignore`, grant this
  routine narrower SSH read access, or drop the routine).
- **PR #33 — "Logic review 2026-09-07: contact-frame velocity gate,
  view-direction gap, outcome_tag clear, calibration rate limit"** (branch
  `logic-review/2026-09-07`, opened 03:13 UTC). Four fixes: (1)
  `find_peak_wrist_frame()` in `compare_swing.py` carried a low-confidence
  wrist position forward as the next frame's velocity baseline, letting a
  single motion-blur frame corrupt contact-frame auto-detection one frame
  later than the existing gate accounted for. (2) `compare_videos.py`'s 1v1
  comparison had no view-direction check, unlike the pro-database path —
  two videos filmed from opposite ends of the court with a similar inferred
  angle could get DTW-compared as if their skeletons were correctly
  aligned. (3) `outcome_tag` couldn't be explicitly cleared via
  `PATCH /highlights/rallies/:id` (the same `??` vs `hasOwnProperty` bug
  already fixed for `boundary_note` in this exact handler). (4)
  `POST /check-setup-live` was missing the per-user rate limit every other
  Python-subprocess/shared-process route already has. 528/528 backend
  tests passing. Python fixes verified by inspection/synthetic script only
  (no `cv2`/`mediapipe` venv in this sandbox) — same standing
  recommendation to run real `pytest` before merging given it touches
  `compare_swing.py`/`compare_videos.py`.
- **PR #34 — "Bug sweep 2026-09-07: reel-file collision, silent-null
  contact detection, billing tier race"** (branch `bug-sweep/2026-09-07`,
  opened 03:31 UTC). Five fixes: (1) the highlight-reel stitcher keyed its
  output filename on `Date.now()` alone, so two same-millisecond reel-build
  requests (e.g. a UI double-tap) could race on one file path and silently
  corrupt the output; now keyed off the unique `reelJobId`. (2) the same
  one-sided-visibility-gate bug PR #33 fixes in `compare_swing.py` also
  exists in `detect_swings.py`'s `compute_wrist_velocity()` (the
  pro-database build side, also reused live by `detect_rallies.py`) — fixed
  there too, with a regression test that actually runs in this sandbox (no
  cv2/mediapipe import) and was confirmed to fail pre-fix. (3)
  `POST /compare-videos` had no user-facing message for a `timeout`
  failure. (4) `POST /billing/sync`'s RevenueCat fetch could resolve after
  a concurrent account deletion, permanently stamping the anonymized row
  `tier='premium'`; now gated on `token_version` matching. (5) the admin
  celebrity-score delete returned `204` on a malformed/nonexistent id
  instead of `400`/`404`. 533/533 backend tests passing; the
  `detect_swings.py` regression test (2/2) actually ran here and was
  confirmed to fail against pre-fix code.
- **PR #35 — "Security review 2026-09-07: rate-limit friend/coach
  invite-code redemption"** (branch `security-review/2026-09-07`, opened
  03:41 UTC). One finding: `POST /friends/link` and `POST /coach/link` had
  no rate limit on their invite-code lookup, unlike every other
  "guess a secret" endpoint in the app — added the same 20-req/15-min
  per-user limiter `analyse.js` uses. Explicitly not exploitable in
  practice (33^8 keyspace against a fully unthrottled worst case), just
  closing the gap for consistency. Everything else re-checked clean
  (JWT/auth, uploads, SQL construction, subprocess spawns, secrets,
  `npm audit`). **No CRITICAL finding.** 527/527 backend tests passing.
- **PR #36 — "Future-ideas brainstorm: 2026-09-07"** (branch
  `future-ideas/2026-09-07`, opened 04:12 UTC). Docs-only, appends a new
  dated section to `docs/future-ideas.md`: self-service data export,
  pre-beta funnel analytics, an accessibility pass, client-side video
  compression before upload, visibility-weighted DTW frame distance, a
  standing regression fixture for the comparison engine, measuring the
  real-world angle-fallback firing rate, a coaching-tip usage audit, an
  orphaned-upload-directory sweep, error tracking/structured logging, and
  subprocess concurrency control ahead of a real beta.

**Not merging any of these myself per standing instructions** — same as
every prior round-up, they're left for Jack's review. See the new
"New from the 2026-09-07 docs round-up" section in `TODO_MANUAL.md` for
what's worth doing about PR #32's structural gap specifically.
