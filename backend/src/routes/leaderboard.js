...
const {
  SHOT_TYPES, MAX_LENGTHS, isShotType, isScore, isText, isOptionalText, isPositiveIntegerId,
...
router.delete('/leaderboard/celebrities/:id', requireAuth, requireAdmin, (req, res) => {
  ...
  const info = db.prepare('DELETE FROM celebrity_scores WHERE id = ?').run(req.params.id);
  if (info.changes === 0) return res.status(404).json({ error: 'Celebrity score not found' });
  res.status(204).end();
});
